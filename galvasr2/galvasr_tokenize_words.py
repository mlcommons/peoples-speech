...
tf.flags.DEFINE_string(
    "out_spelling_numbers_txt",
    None,
    "Name of output file. Will be in Kaldi's lexicon_numbers.txt format",
)
tf.flags.DEFINE_string(
    "out_units_txt", None, "Name of output file. Will be in Kaldi's units.txt format"
)
tf.flags.DEFINE_string(
    "space_char", None, 'Space charactr. " " is invalid for openfst.'
)
...
def dump_spellings():
    ...
    def tokenize_words(words_t):
        ...
        ragged_tokenized_t = tf.RaggedTensor.from_tensor(
            padded_tokenized_t, lengths=lengths_t
        )
    ...
    with open(FLAGS.out_spelling_txt, "w") as spelling_fh, open(
        FLAGS.out_spelling_numbers_txt, "w"
    ) as spelling_numbers_fh:
