...
def add_samples(sdb_writer):
    ...
    for sample in progress(
        change_audio_types(
            samples, audio_type=sdb_writer.audio_type, processes=CLI_ARGS.workers
        ),
        total=len(samples),
...
def handle_args():
    ...
    parser.add_argument(
        "--sort",
        action="store_true",
        help="Force sample sorting by durations " "(assumes SDB sources unsorted)",
    )
    parser.add_argument(
        "--sort-tmp-file",
        default=None,
        help='Overrides default tmp_file (target + ".tmp") '
        "for sorting through --sort option",
    )
    parser.add_argument(
        "--sort-cache-size",
        default="1GB",
        help="Cache (bucket) size for binary audio data "
        "for sorting through --sort option",
    )
    parser.add_argument(
        "--no-progress",
