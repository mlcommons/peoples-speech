...
def main(args):
    parser = argparse.ArgumentParser(
        description="Annotate .tlog or .script files by adding meta data"
    )
    parser.add_argument("target", type=str, help="")
    parser.add_argument(
        "assignments", nargs="+", help="Meta data assignments of the form <key>=<value>"
    )
    ...
    for assignment in args.assignments:
        ...
        if key in forbidden_keys:
            print(
                'Meta data key "{}" not allowed - forbidden: {}'.format(
                    key, "|".join(forbidden_keys)
                )
            )
            sys.exit(1)
