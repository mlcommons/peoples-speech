...
def build_catalog():
        ...
        for item in catalog_items:
            ...
            for entry, entry_original_path in item.items():
                ...
                entry_path = (
                    ...
                    else (base_path / entry_path).absolute()
                )
                if (
                    (len(CLI_ARGS.check) == 1 and CLI_ARGS.check[0] == "all")
                    or entry in CLI_ARGS.check
                ) and not entry_path.is_file():
    ...
    if CLI_ARGS.output is not None:
        ...
        if CLI_ARGS.order_by is not None:
            items.sort(
                key=lambda i: i[CLI_ARGS.order_by] if CLI_ARGS.order_by in i else ""
            )
...
def handle_args():
    parser = argparse.ArgumentParser(
        description="Tool for combining catalog files and/or ordering, checking and "
        "converting paths within catalog files"
    )
    parser.add_argument(
        "--output", help="Write collected catalog items to this new catalog file"
    )
    parser.add_argument(
        "--make-relative",
        action="store_true",
    ...
    parser.add_argument(
        "--on-miss",
        default="fail",
        choices=["fail", "drop", "remove", "ignore"],
    ...
    parser.add_argument(
        "--order-by", help="Path entry used for sorting items in target catalog"
    )
    parser.add_argument(
        "sources", nargs="+", help="Source catalog files (supporting wildcards)"
    )
