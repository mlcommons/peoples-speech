...
class AlignmentStatistics:
    ...
    def load_catalog(self, catalog_path, ignore_missing=True):
        ...
        for entry in AlignmentStatistics.progress(
            catalog_entries, desc="Reading catalog"
        ):
    ...
    def print_stats(self):
        ...
        print("")
        print(
            "Total aligned utterance character length: {:,}".format(self.total_length)
        )
        print("")
        print(
            "Total utterance duration: {} ({:,} hours)".format(
        ...
        for meta_type, counter in self.meta_counters.items():
            print(
                'Overall number of instances of meta type "{}": {:,}'.format(
                    meta_type, len(counter.keys())
                )
            )
            print("")
...
def main(args):
    ...
    parser.add_argument(
        "--no-progress", action="store_true", help="Prevents showing progress bars"
    )
    parser.add_argument(
        "--progress-interval",
        type=float,
