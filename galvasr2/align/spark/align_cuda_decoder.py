...
def main(argv):
  ...
  """
    ...
    if FLAGS.stage <= 2:
        ...
        alphabet_set = set()
        with open(
            "/opt/kaldi/egs/aspire/s5/exp/tdnn_7b_chain_online/graph_pp/words.txt", "r"
        ) as fh:
            for line in fh:
                word = line.split(" ")[0]
        ...
        alignments_df.write.mode("overwrite").format("json").save(
            os.path.join(FLAGS.work_dir, "alignments_json")
        )
    ...
    if FLAGS.stage <= 3:
        ...
        create_audio_segments_udf = prepare_create_audio_segments_udf(
            gs_bucket=FLAGS.input_gcs_bucket,
        ...
        output_df.write.mode("overwrite").json(
            os.path.join(FLAGS.work_dir, "dataset_manifest_wav")
        )
