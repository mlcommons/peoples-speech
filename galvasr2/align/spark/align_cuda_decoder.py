...
def main(argv):
  ...
  if FLAGS.stage <= 2:
    ...
    align_udf = prepare_align_udf(dsalign_args, alphabet_normalized_path, 15_000, 3_000)
    ...
    alignments_df.write.mode("overwrite").format("json").save(os.path.join(FLAGS.work_dir, "alignments_json_jul_28"))
  ...
  if FLAGS.stage <= 3:
    alignments_df = spark.read.json(os.path.join(FLAGS.work_dir, "alignments_json_jul_28"))
    ...
    alignments_df = alignments_df.withColumn("duration_ms",
                                             F.expr("transform(arrays_zip(alignments.end_ms, alignments.start_ms), x -> x.end_ms - x.start_ms)"))
    alignments_df = alignments_df.withColumn("alignments", F.arrays_zip(alignments_df.alignments.cer, alignments_df.alignments.end_ms,
                                                  alignments_df.alignments.label, alignments_df.alignments.start_ms,
                                                  alignments_df.alignments.wer, alignments_df.duration_ms).cast(
                                                      T.ArrayType(T.StructType([T.StructField("cer", T.FloatType()),
    ...
    alignments_df = alignments_df.withColumn("alignments",
                                             F.filter(alignments_df.alignments, lambda alignment: (alignment.duration_ms < FLAGS.max_duration_ms) &
                                                      (alignment.cer < FLAGS.max_cer)))
    alignments_df = alignments_df.withColumn("alignments", F.struct(alignments_df.alignments.cer, alignments_df.alignments.end_ms,
                                                                    alignments_df.alignments.label, alignments_df.alignments.start_ms,
                                                                    alignments_df.alignments.wer, alignments_df.duration_ms).cast(
                                                                        T.StructType([T.StructField("cer", T.ArrayType(T.FloatType())),
    ...
    abc = alignments_df.select(F.sum(F.expr("aggregate(alignments.duration_ms, 0L, (x, acc) -> acc + x)")) / 1000. / 60. / 60.).collect()
    print("GALVEZ:total number of hours=", abc)
    
    alignments_df = alignments_df.select(alignments_df.identifier,
    ...
    audio_df = spark.read.format("binaryFile").load(paths).drop("modificationTime", "length")
    ...
    name = F.concat(F.col("identifier"),
                    F.lit("-"),
                    F.col("audio_document_id")
    )
    ...
    alignments_audio_df = alignments_audio_df.withColumn("aligned_chunks",
                                   create_audio_segments_udf(alignments_audio_df.content,
                                                             F.lit("mp3"),
                                                             name,
                                                             alignments_audio_df.alignments.start_ms,
                                                             alignments_audio_df.alignments.end_ms))
    a = alignments_audio_df.select(F.explode(F.arrays_zip("aligned_chunks.audio_name", "aligned_chunks.audio"))).select("col.0", "col.1")
    ...
    output_df = alignments_audio_df.select(
        ...
        F.struct(
            ...
            create_audio_segment_names_udf(name, F.size(alignments_audio_df.alignments.start_ms)).alias("name"),
  ...
  repartitioned_tars_dir = os.path.join(FLAGS.work_dir, "repartitioned_dataset_tars_jul_28_flac_new_join")
  ...
  tmp_tars_dir = os.path.join(FLAGS.work_dir, "repartitioned_dataset_tars_jul_28_tmp_dir2_new_join_flac")
  if FLAGS.stage <= 4:
    ...
    spark2.conf.set("spark.sql.execution.rangeExchange.sampleSizePerPartition", number_of_rows)
    ...
    tars_df = tars_df.repartitionByRange(1024, F.col("uuid")) # F.col("uuid"))  # F.monotonically_increasing_id())
    ...
    tars_df = spark2.read.format("tar").load(tmp_tars_dir) # .repartitionByRange()  # coalesce(1024)
    counts_df = (tars_df.withColumn("partitionId", F.spark_partition_id())
                 .groupBy("partitionId")
                 .count())
    ...
    tars_df = spark2.createDataFrame(tars_df.rdd.mapPartitions(drop_final_rows), schema=tars_df.schema)
  ...
  nemo_manifest_dir = os.path.join(FLAGS.work_dir, "dataset_manifest_nemo_jul_28_flac_filtered_new_join")
  nemo_single_manifest_dir = os.path.join(FLAGS.work_dir, "dataset_manifest_nemo_jul_28_flac_filtered_single_new_join")

  if FLAGS.stage <= 5:
    json_df = spark.read.format("json").load(manifest_dir)
    nemo_df = json_df.select(
        F.explode(F.arrays_zip(F.col("training_data.name").alias("audio_filepath"),
                               F.col("training_data.label").alias("text"),
                               F.col("training_data.duration_ms").alias("duration_ms"))))
    nemo_df = nemo_df.select(F.col("col.name").alias("audio_filepath"),
                             F.col("col.label").alias("text"),
                             (F.col("col.duration_ms").cast(T.DoubleType()) / 1000.).alias("duration"),
    ...
    nemo_df = nemo_df.join(tars_df, F.col("audio_filepath") == F.col("key")).drop(F.col("key"))
    ...
    nemo_single_df.coalesce(1).write.mode("overwrite").format("json").save(nemo_single_manifest_dir)

  single_manifest_dir = os.path.join(FLAGS.work_dir, "dataset_manifest_jul_28_flac_single_new_join")
  single_tar_dir = os.path.join(FLAGS.work_dir, "dataset_tars_jul_28_flac_single_new_join")
  # Create single tar file and single json file
  if FLAGS.stage <= 6:
    json_df = spark.read.format("json").load(manifest_dir)
    json_df.coalesce(1).write.format("json").mode("overwrite").save(single_manifest_dir)
