WAV_FROM_MP3_TYPE = T.StructType([T.StructField("audio_name", T.StringType()),
                                           T.StructField("audio", T.BinaryType())])
@F.pandas_udf(WAV_FROM_MP3_TYPE)
def wav_from_mp3_udf(audio_bytes_series: pd.Series, audio_type_series: pd.Series,
                     audio_name_series: pd.Series) -> pd.DataFrame:
    output_array = []
