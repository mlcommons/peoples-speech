...
def main(argv):
    spark = (
        SparkSession.builder.master("local[15]")
    ...
    audio_document_id_int64_id_dir = os.path.join(
        FLAGS.work_dir, "audio_document_id_to_int64_id"
    )
    if FLAGS.stage <= 0:
        audio_df = load_audio_files(spark, training_sample_rows, FLAGS.input_dir)
        vad_udf = prepare_vad_udf(
            num_padding_frames=10, threshold=0.5, aggressiveness=0, frame_duration_ms=30
        )
        vad_df = audio_df.withColumn(
            "vad",
            vad_udf(audio_df.content, audio_df.format, audio_df.audio_document_id),
        )
        vad_df = vad_df.withColumn(
            "num_utterances_in_audio_document", F.size(vad_df.vad.voiced_buffer)
        )

        exploded_voiced_buffer_df = vad_df.select(
            vad_df.audio_document_id,
        ...
        ).write.mode("overwrite").format("json").save(audio_document_id_int64_id_dir)
    ...
    if FLAGS.stage <= 2:
        ...
        log_probabilities_schema = StructType(
            [
                StructField("int64_uttid", LongType()),
        ...
        log_probabilities_df = (
            spark.read.format("tfrecord")
            .schema(log_probabilities_schema)
            .load(logits_dir)
        )
        # log_probabilities_df = log_probabilities_df.groupBy(log_probabilities_df.int64_audio_document_id).

        audio_document_integer_mapping_df = spark.read.format("json").load(
            audio_document_id_int64_id_dir
        )
        log_probabilities_df = log_probabilities_df.join(
            audio_document_integer_mapping_df, "int64_audio_document_id"
        )
        ...
        df = (
            transcripts_df.join(catalogue_df, ["text_document_id"])
            .join(log_probabilities_df, ["audio_document_id"])
            .drop("int64_audio_document_id", "text_document_format")
        )
