...
def DecodeToWavPipe(input_bytes, fmt):
    cmd = (
        f"sox -t {fmt} - -t wav --channels 1 --rate 16000 --encoding signed --bits 16 -"
    )
    p = subprocess.Popen(
        shlex.split(cmd),
...
def file_exists_udf(path_series: pd.Series) -> pd.Series:
    ...
    with ThreadPoolExecutor(80) as executor:
        return pd.Series(
            list(
                tqdm.tqdm(
                    executor.map(tf.io.gfile.exists, path_series),
                    total=len(path_series),
                )
            )
        )


FUZZY_MATCHING_RETURN_TYPE = StructType(
    [
    ...
    def __exit__(self, exc, value, tb):
        try:
            ...
            try:
                ...
                unmounted = False
                print(
                    f"WARNING: Failed to unmount {self.name}. Not removing temporary directory. Trying to remove a temporary directory that fails to unmount results in: OSError: [Errno 5] Input/output error"
                )
        if unmounted:
    ...
    def get_soxi_info_udf(audio_file_series: pd.Series) -> pd.Series:
        ...
        with TemporaryMountDirectory(
            mount_cmd=[
                "gcsfuse",
                "--implicit-dirs",
                "gs://the-peoples-speech-west-europe".lstrip("gs://"),
            ],
            unmount_cmd=["fusermount", "-u"],
        ) as temp_dir_name:
            for audio_file in audio_file_series:
                audio_file = re.sub(
                    r"^{0}".format("gs://the-peoples-speech-west-europe"),
                    temp_dir_name,
                    audio_file,
                )
                ...
                try:
                    duration = subprocess.check_output(
                        shlex.split(cmd), stderr=subprocess.DEVNULL, timeout=10
                    )  # 10 second timeout
                    # print("GALVEZ:value=")
                    # print(duration)
...
def prepare_create_audio_segments_udf(gs_bucket: str, output_dir: str):
    ...
    @pandas_udf(RETURN_TYPE)
    def create_audio_segments_udf(
        audio_file_gcs_paths: pd.Series,
        ...
        with TemporaryMountDirectory(
            ...
            if not output_dir.startswith("gs://"):
                ...
                posix_output_dir = re.sub(
                    r"^{0}".format(gs_bucket), temp_dir_name, output_dir
                )
            for (
                audio_file_gcs_path,
                identifier,
                ...
                audio_file_path = re.sub(
                    r"^{0}".format(gs_bucket), temp_dir_name, audio_file_gcs_path
                )
                print(f"GALVEZ:audio_file_path={audio_file_path}")
                try:
                    source = AudioSegment.from_file(
                        audio_file_path, subprocess_timeout=100
                    )
                except subprocess.TimeoutExpired:
                    print("GALVEZ:timed out, need to retry")
                    return create_audio_segments_udf.func(
                        audio_file_gcs_paths,
                        identifier_series,
                        audio_document_id_series,
                        start_ms_arrays,
                ...
                already_done = timeout(
                    os.path.exists,
                    (os.path.join(this_file_output_dir, last_write_file_name),),
                    timeout_duration=100,
                )
                if already_done:
                    pass
                for i, (start_ms, end_ms) in enumerate(
                    ...
                    if not already_done:
                        ...
                        try:
                            ...
                            print(
                                f"GALVEZ: Couldn't encode {write_path} [{start_ms}:{end_ms}]"
                            )
                            continue
                        else:
                            fh.close()
