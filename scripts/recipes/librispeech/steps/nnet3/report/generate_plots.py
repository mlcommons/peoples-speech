...
def get_args():
    ...
    parser.add_argument(
        "output_dir", help="output directory for reports, e.g. 'exp/nnet3/tdnn/report'"
    )
...
"""
    ...
    def compile(self):
        ...
        try:
            ...
            logger.warning(
                "There was an error compiling LaTeX file %s. "
...
def generate_acc_logprob_plots(
    exp_dir,
    output_dir,
    plot,
    key="accuracy",
    file_basename="accuracy",
    comparison_dir=None,
    start_iter=1,
    latex_report=None,
    output_name="output",
    ...
    index = 0
    for dir in dirs:
        [report, times, data] = log_parse.generate_acc_logprob_report(
            dir, key, output_name
        )
        if index == 0:
            # this is the main experiment directory
            with open("{0}/{1}.log".format(output_dir, file_basename), "w") as f:
                f.write(report)
        ...
        if plot:
            ...
            if data.shape[0] == 0:
                logger.warning(
                    "Couldn't find any rows for the"
            ...
            (plot_handle,) = plt.plot(
                data[:, 0], data[:, 2], color=color_val, label="valid {0}".format(dir)
            )
            plots.append(plot_handle)
        index += 1
    if plot:
        ...
        figfile_name = "{0}/{1}_{2}.pdf".format(
            output_dir, file_basename, latex_compliant_name(output_name)
        )
...
def insert_a_column_legend(
    legend_handle, legend_label, lp, mp, hp, dir, prefix_length, column_index
):
...
def plot_a_nonlin_component(
    ...
    if not with_oderiv:
        for dir in dirs:
            ...
            (lp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 5],
                color=color_val,
                linestyle="--",
            )
            (mp,) = ax.plot(
                data[:, 0], data[:, gate_index * 10 + 6], color=color_val, linestyle="-"
            )
            (hp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 7],
                color=color_val,
                linestyle="--",
            )
            insert_a_column_legend(
                legend_handle, legend_label, lp, mp, hp, dir, prefix_length, index + 1
            )
            ...
            (lp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 8],
                color=color_val,
                linestyle="--",
            )
            (mp,) = ax.plot(
                data[:, 0], data[:, gate_index * 10 + 9], color=color_val, linestyle="-"
            )
            (hp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 10],
                color=color_val,
                linestyle="--",
            )
        ...
        lgd = plt.legend(
            ...
            ncol=4,
            handletextpad=-2,
            title="[1]:{0}".format(common_prefix),
            borderaxespad=0.0,
        )
        plt.grid(True)
        ...
        for dir in dirs:
            ...
            (mp,) = ax.plot(
                data[:, 0], data[:, gate_index * 10 + 8], color=color_val, linestyle="-"
            )
            (hp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 9],
                color=color_val,
                linestyle="--",
            )
            insert_a_column_legend(
            ...
            (mp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 11],
                color=color_val,
                linestyle="-",
            )
            (hp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 12],
                color=color_val,
                linestyle="--",
            )
            ax.set_ylabel("Derivative-{0}".format(component_type))
            ...
            (lp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 13],
                color=color_val,
                linestyle="--",
            )
            (mp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 14],
                color=color_val,
                linestyle="-",
            )
            (hp,) = ax.plot(
                data[:, 0],
                data[:, gate_index * 10 + 15],
                color=color_val,
                linestyle="--",
            )
            ax.set_xlabel("Iteration")
...
def generate_nonlin_stats_plots(
    exp_dir, output_dir, plot, comparison_dir=None, start_iter=1, latex_report=None
):
    assert start_iter >= 1
    ...
    for component_name in main_stat_tables.keys():
        ...
        with open(
            "{dir}/nonlinstats_{comp_name}.log".format(
                dir=output_dir, comp_name=component_name
            ),
            "w",
        ) as f:
            if with_oderiv:
    ...
    if plot:
        ...
        if plot_component_names != main_component_names:
            logger.warning(
                "The components in all the neural networks in the "
                ...
                for i in range(0, 5):
                    ...
                    fig.suptitle(
                        ...
                        "{component_name}-{gate}".format(
                            component_name=component_name, gate=g_lstm_gate[i]
                        )
                    )
                    ...
                    fig.savefig(
                        figfile_name, bbox_extra_artists=(lgd,), bbox_inches="tight"
                    )
                    if latex_report is not None:
                        latex_report.add_figure(
                            figfile_name,
...
def generate_clipped_proportion_plots(
    exp_dir, output_dir, plot, comparison_dir=None, start_iter=1, latex_report=None
):
    assert start_iter >= 1
    ...
    for dir in dirs:
        try:
            stats_per_dir[dir] = log_parse.parse_progress_logs_for_clipped_proportion(
                dir
            )
            ...
            logger.warning(
                "Could not extract the clipped proportions for %s, "
                "this might be because there are no ClipGradientComponents.",
                dir,
            )
            continue
    ...
    try:
        ...
        logger.warning(
            "The main experiment directory %s does not have clipped proportions. "
            "Not generating clipped proportion plots.",
            exp_dir,
        )
        return
    ...
    if plot:
        main_component_names = sorted(
            stats_per_dir[exp_dir]["cp_per_iter_per_component"]
        )
        plot_component_names = set(main_component_names)
        for dir in dirs:
            try:
        ...
        for component_name in main_component_names:
            ...
            for dir in dirs:
                ...
                index += 1
                try:
                    iter_stats = stats_per_dir[dir]["cp_per_iter_per_component"][
                        component_name
                    ]
                except KeyError:
                    # this component is not available in this network so lets
            ...
            plt.grid(True)
            fig.suptitle(
                "Clipped-proportion value at {comp_name}".format(
            ...
            if latex_report is not None:
                latex_report.add_figure(
                    figfile_name, "Clipped proportion at {0}".format(component_name)
                )


def generate_parameter_diff_plots(
    exp_dir, output_dir, plot, comparison_dir=None, start_iter=1, latex_report=None
):
    # Parameter changes
    assert start_iter >= 1
    ...
    for dir in dirs:
        stats_per_dir[dir] = {}
        for key in key_file:
            stats_per_dir[dir][key] = log_parse.parse_progress_logs_for_param_diff(
                dir, key
            )
    ...
    for diff_type in key_file:
        with open("{0}/{1}".format(output_dir, key_file[diff_type]), "w") as f:
            diff_per_component_per_iter = stats_per_dir[exp_dir][diff_type][
            ...
            for iter in range(max_iter + 1):
                ...
                for c in component_names:
                    try:
                        ...
                        iter_data.append("NA")
                if (
                    float(total_missing_iterations) / len(component_names) > 20
                    and not gave_user_warning
                ):
                    logger.warning(
                        "There are more than %.0f missing iterations per component. "
    ...
    if plot:
        ...
        for dir in dirs:
            try:
                component_names = set(
                    stats_per_dir[dir][diff_type]["progress_per_component"]
                )
                plot_component_names = plot_component_names.intersection(
                    component_names
                )
        ...
        fig = plt.figure()
        logger.info(
            "Plotting parameter differences for components: "
        ...
        for component_name in main_component_names:
            ...
            for dir in dirs:
                ...
                try:
                    ...
                    if dir == exp_dir:
                        raise Exception(
                            "No parameter differences were available even in the main "
                ...
                (mp,) = ax.plot(
                    ...
                    iter_stats[1][:, 1],
                    color=color_val,
                    label="Relative Parameter " "Differences {0}".format(dir),
                )
                ax.set_xlabel("Iteration")
            ...
            plt.grid(True)
            fig.suptitle(
                "Parameter differences at {comp_name}".format(comp_name=component_name)
            )
            comp_name = latex_compliant_name(component_name)
            ...
            if latex_report is not None:
                latex_report.add_figure(
                    figfile_name, "Parameter differences at {0}".format(component_name)
                )


def generate_plots(
    exp_dir, output_dir, output_names, comparison_dir=None, start_iter=1
):
    try:
    ...
    for (output_name, objective_type) in output_names:
        if objective_type == "linear":
            ...
            logger.info(
                "Generating %s objective plots for '%s'", objective_type, output_name
            )
            generate_acc_logprob_plots(
                exp_dir,
                output_dir,
    ...
    if g_plot and latex_report is not None:
        ...
        if has_compiled:
            logger.info(
                "Report file %s/report.pdf has been generated successfully.", output_dir
            )
