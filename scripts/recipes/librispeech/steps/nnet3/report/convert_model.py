...
def read_float(s, pos):
    ...
    try:
        ...
        print(
            "{0}: at file position {1}, expected float but got {1}".format(
...
def read_int(s, pos):
    ...
    try:
        ...
        print(
            "{0}: at file position {1}, expected int but got {1}".format(tok).format(
                sys.argv[0], orig_pos, tok
            ),
            file=sys.stderr,
        )
...
def read_vector(s, pos):
    ...
    if tok != "[":
        print(
            "{0}: at file position {1}, expected vector but got {1}".format(tok).format(
                sys.argv[0], pos, tok
            ),
            file=sys.stderr,
        )
    ...
    while True:
        ...
        try:
            ...
            print(
                "{0}: at file position {1}, reading vector, expected float but got {1}".format(
                    sys.argv[0], pos, tok
                ),
                file=sys.stderr,
            )
    ...
    if tok is None:
        print(
            "{0}: encountered EOF while reading vector.".format(tok).format(
                sys.argv[0]
            ),
            file=sys.stderr,
        )
...
def read_matrix(s, pos):
    ...
    if tok != "[":
        print(
            "{0}: at file position {1}, expected matrix but got {1}".format(tok).format(
                sys.argv[0], pos, tok
            ),
            file=sys.stderr,
        )
    ...
    while True:
        ...
        while True:
            ...
            if tok == "]" or tok == None:
                ...
                try:
                    ...
                    print(
                        "{0}: at file position {1}, reading matrix, expected float but got {2}".format(
                            sys.argv[0], pos, tok
                        ),
                        file=sys.stderr,
                    )
        ...
        if tok == "None":
            print(
                "{0}: matrix starting at position {1} was unexpectedly terminated by EOF.".format(
                    sys.argv[0], pos
                ),
                file=sys.stderr,
            )
    ...
    try:
        ...
        if tok is None:
            print(
                "{0}: error converting matrix starting at position {1} into numpy array.".format(
                    sys.argv[0], orig_pos
                ),
                file=sys.stderr,
            )
...
def read_model(filename):
    ...
    while True:
        ...
        if tok is None:
            print(
                "{0}: unexpected EOF on output of command {1}".format(
                    sys.argv[0], command
                )
            )
            return None
    ...
    for c in range(num_components):
        ...
        if component != None:
            ...
            print(
                "{0}: error reading component with name {1} at position {2}".format(
                    sys.argv[0], component_name, component_pos
                ),
                file=sys.stderr,
            )
...
def test():
    ...
    assert check_for_newline("hello \nthere", 5) == (True, 7)
    (m, pos) = read_matrix(" [\n 1 2 3\n 4 5 6 ] ", 0)
    assert pos == 18 and np.array_equal(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            "Usage: {0} <nnet3-model-in> <pickled-model-out>".format(sys.argv[0]),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if m != None:
        try:
            ...
            print("{0}: error writing to {1}".format(sys.argv[2]), file=sys.stderr)
