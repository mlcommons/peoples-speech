...
def train(args, run_opts):
        ...
        if args.use_dense_targets:
            ...
            try:
                ...
                if common_lib.get_feat_dim_from_scp(args.targets_scp) != num_targets:
                    raise Exception(
                        "Mismatch between num-targets provided to " "script vs configs"
                    )
            except KeyError as e:
                num_targets = -1
        else:
            ...
            try:
                ...
                raise Exception(
                    "KeyError {0}: Variables need to be defined "
                    "in {1}".format(str(e), "{0}/configs".format(args.dir))
                )
    ...
    if args.chunk_width != frames_per_eg_str:
        raise Exception(
            ...
            "in the egs dir {0} vs {1}".format(args.chunk_width, frames_per_eg_str)
        )

    if args.num_jobs_final > num_archives:
        raise Exception(
            "num_jobs_final cannot exceed the number of archives "
    ...
    num_iters = int(
        (num_archives_to_process * 2) / (args.num_jobs_initial + args.num_jobs_final)
    )
    ...
    if os.path.exists("{0}/valid_diagnostic.scp".format(egs_dir)):
        if os.path.exists("{0}/valid_diagnostic.egs".format(egs_dir)):
            raise Exception(
                "both {0}/valid_diagnostic.egs and "
        ...
        if not os.path.exists("{0}/valid_diagnostic.egs" "".format(egs_dir)):
            raise Exception(
                "neither {0}/valid_diagnostic.egs nor "
                "{0}/valid_diagnostic.scp exist."
    ...
    if args.stage <= num_iters:
        if args.do_final_combination:
            ...
            common_lib.force_symlink(
                "{0}.raw".format(num_iters), "{0}/final.raw".format(args.dir)
            )
