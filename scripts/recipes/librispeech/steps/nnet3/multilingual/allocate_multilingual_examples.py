...
def get_args():
    ...
    parser.add_argument(
        "egs_dir", help="Name of output egs directory e.g. exp/tdnn_multilingual_sp/egs"
    )
...
def process_multilingual_egs(args):
    ...
    for lang in range(num_langs):
        ...
        logger.info(
            "Number of examples for language {0} "
    ...
    with open(
        "{0}/info/{1}num_archives".format(args.egs_dir, args.egs_prefix), "w"
    ) as fh:
        print("{0}".format(num_archives), file=fh)

    logger.info(
        "There are a total of {} examples in the input scp "
        "files.".format(tot_num_egs)
    )
    logger.info(
        "Number of blocks in each output archive will be approximately "
        "{}, and block-size is {}.".format(
            int(round(tot_num_egs / num_archives / args.block_size)), args.block_size
        )
    )
    for lang in range(num_langs):
        blocks_per_archive_this_lang = (
            lang_to_num_examples[lang] / num_archives / args.block_size
        )
        warning = ""
        if blocks_per_archive_this_lang < 1.0:
            warning = (
                "Warning: This means some of the output archives might "
                "not include any examples from this lang."
            )
        logger.info(
            "The proportion of egs from lang {} is {:.2f}. The number of blocks "
            "per archive for this lang is approximately {:.2f}. "
            "{}".format(
                lang,
                float(lang_to_num_examples[lang]) / tot_num_egs,
                blocks_per_archive_this_lang,
                warning,
            )
        )
    ...
    for archive_index in range(
        num_archives + 1
    ):  #  +1 is because we write to the last archive in two rounds
        ...
        if not last_round:
            num_blocks_this_archive = int(
                round(float(num_remaining_blocks) / num_remaining_archives)
            )
            logger.info(
                "Generating archive {} containing {} blocks...".format(
                    archive_index, num_blocks_this_archive
                )
            )
            ...
            logger.info(
                "Writing all the {} remaining egs to the last archive...".format(
                    num_remaining_egs
                )
            )

        out_scp_file_handle = open(
            "{0}/{1}{2}.scp".format(args.egs_dir, args.egs_prefix, archive_index + 1),
            "a" if last_round else "w",
        )
        eg_to_output_file_handle = open(
            "{0}/{1}output.{2}.ark".format(
                args.egs_dir, args.egs_prefix, archive_index + 1
            ),
            "a" if last_round else "w",
        )
        eg_to_weight_file_handle = open(
            "{0}/{1}weight.{2}.ark".format(
                args.egs_dir, args.egs_prefix, archive_index + 1
            ),
            "a" if last_round else "w",
        )
        ...
        for block_index in range(num_blocks_this_archive):
            ...
            remaining_proportions = [
                float(remain) / tot
                for remain, tot in zip(lang_to_num_remaining_egs, lang_to_num_examples)
            ]
            lang_index, max_proportion = max(
                enumerate(remaining_proportions), key=lambda a: a[1]
            )
            ...
            for eg_line in example_lines:
                ...
                print(
                    "{0} output-{1}".format(eg_id, lang_index),
                    file=eg_to_output_file_handle,
                )
                print(
                    "{0} {1}".format(eg_id, lang2weight[lang_index]),
                    file=eg_to_weight_file_handle,
                )
    ...
    for handle in in_scp_file_handles:
        handle.close()
    logger.info(
        "Finished generating {0}*.scp, {0}output.*.ark "
        "and {0}weight.*.ark files. Wrote a total of {1} examples "
        "to {2} archives.".format(
            args.egs_prefix, tot_num_egs - num_remaining_egs, num_archives
        )
    )
