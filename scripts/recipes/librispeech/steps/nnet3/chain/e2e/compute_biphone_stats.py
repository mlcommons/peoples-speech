...
print(
    "Loaded {} phone-sets containing {} phones.".format(len(phone_sets), len(phones)),
    file=sys.stderr,
)
...
for line in sys.stdin:
    ...
    line_phones = line[1:]
    for pair in zip(
        [0] + line_phones, line_phones
    ):  # 0 is for the no left-context case
...
for phone1 in [0] + phones:
    for phone2 in phones:
        pair = (phone1, phone2)
        shared_pair = (
            (phone_to_shard_phone[pair[0]], phone_to_shard_phone[pair[1]])
            if args.shared_phones
            else pair
        )
