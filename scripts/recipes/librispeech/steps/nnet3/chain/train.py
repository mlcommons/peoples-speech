...
def get_args():
    ...
    parser.add_argument(
        "--chain.right-tolerance", type=int, dest="right_tolerance", default=5, help=""
    )
    parser.add_argument(
        "--chain.left-tolerance", type=int, dest="left_tolerance", default=5, help=""
    )
    parser.add_argument(
        "--chain.leaky-hmm-coefficient",
        type=float,
    ...
    parser.add_argument(
        "--chain.frame-subsampling-factor",
        type=int,
    ...
    parser.add_argument(
        "--chain.alignment-subsampling-factor",
        type=int,
    ...
    parser.add_argument(
        "--trainer.optimization.shrink-value",
        type=float,
        dest="shrink_value",
        default=1.0,
...
def process_args(args):
    ...
    if args.left_deriv_truncate is not None:
        ...
        logger.warning(
            ...
            "--trainer.deriv-truncate-margin.".format(args.deriv_truncate_margin)
        )

    if not os.path.exists(args.dir):
        raise Exception(
            "Directory specified with --dir={0} " "does not exist.".format(args.dir)
        )
    ...
    if args.use_gpu in ["yes", "wait"]:
        ...
        logger.warning(
            "Without using a GPU this will be very slow. "
    ...
    run_opts.egs_command = (
        args.egs_command if args.egs_command is not None else args.command
    )
...
def train(args, run_opts):
    ...
    chain_lib.check_for_required_files(
        args.feat_dir, args.tree_dir, args.lat_dir if args.egs_dir is None else None
    )
    ...
    common_lib.execute_command(
        "utils/split_data.sh {0} {1}" "".format(args.feat_dir, num_jobs)
    )
    ...
    try:
        ...
        raise Exception(
            "KeyError {0}: Variables need to be defined in "
    ...
    left_context_initial = (
        args.chunk_left_context_initial + model_left_context
        if args.chunk_left_context_initial >= 0
        else -1
    )
    right_context_final = (
        args.chunk_right_context_final + model_right_context
        if args.chunk_right_context_final >= 0
        else -1
    )
    ...
    if args.stage <= -6:
        ...
        chain_lib.create_phone_lm(
            args.dir, args.tree_dir, run_opts, lm_opts=args.lm_opts
        )
    ...
    if (
        ...
        logger.info(
            "Initializing a basic network for estimating " "preconditioning matrix"
        )
        common_lib.execute_command(
    ...
    egs_left_context_initial = (
        left_context_initial + args.frame_subsampling_factor // 2
        if left_context_initial >= 0
        else -1
    )
    egs_right_context_final = (
        right_context_final + args.frame_subsampling_factor // 2
        if right_context_final >= 0
        else -1
    )

    default_egs_dir = "{0}/egs".format(args.dir)

    if (args.egs_dir is not None) and (
        args.cmvn_opts != "--norm-means=false --norm-vars=false"
    ):
        logger.warning(
    ...
    if args.num_jobs_final > num_archives_expanded:
        raise Exception(
            "num_jobs_final cannot exceed the " "expanded number of archives"
        )
    ...
    if not os.path.exists("{0}/valid_diagnostic.cegs".format(egs_dir)):
        if not os.path.exists("{0}/valid_diagnostic.scp".format(egs_dir)):
            raise Exception(
                "Neither {0}/valid_diagnostic.cegs nor "
    ...
    if args.stage <= -1:
        ...
        chain_lib.prepare_initial_acoustic_model(
            args.dir, run_opts, input_model=args.input_model
        )
    ...
    if args.do_final_combination:
        models_to_combine = common_train_lib.get_model_combine_iters(
            num_iters,
            args.num_epochs,
            num_archives_expanded,
            args.max_models_combine,
            args.num_jobs_final,
        )
    else:
    ...
    logger.info(
        "Training will run for {0} epochs = "
        ...
        if args.stage <= iter:
            ...
            if shrinkage_value <= 0.5:
                raise Exception(
                    ...
                    "shrink-value={1}".format(args.proportional_shrink, shrinkage_value)
                )
            if args.shrink_value < shrinkage_value:
                shrinkage_value = (
                    args.shrink_value
                    if common_train_lib.should_do_shrinkage(
                        iter, model_file, args.shrink_saturation_threshold
                    )
            ...
            if args.email is not None:
                ...
                if iter % reporting_iter_interval == 0:
                    ...
                    message = report
                    subject = "Update : Expt {dir} : " "Iter {iter}".format(
    ...
    if args.stage <= num_iters:
        if args.do_final_combination:
            ...
            common_lib.force_symlink(
                "{0}.mdl".format(num_iters), "{0}/final.mdl".format(args.dir)
            )
            ...
            common_lib.force_symlink(
                "compute_prob_valid.{iter}.log" "".format(iter=num_iters),
    ...
    if args.email is not None:
        common_lib.send_mail(
            report, "Update : Expt {0} : " "complete".format(args.dir), args.email
        )

    with open("{dir}/accuracy.report".format(dir=args.dir), "w") as f:
        f.write(report)

    common_lib.execute_command("steps/info/chain_dir_info.pl " "{0}".format(args.dir))
...
def main():
    ...
    try:
        ...
        if args.email is not None:
            message = (
                "Training session for experiment {dir} "
