...
parser = argparse.ArgumentParser(
    description="Writes config files and variables " "for TDNNs creation and training",
...
parser.add_argument("--feat-dim", type=int, help="Raw feature dimension, e.g. 13")
parser.add_argument(
...
parser.add_argument("--relu-dim", type=int, help="dimension of ReLU nonlinearities")
parser.add_argument(
...
parser.add_argument("config_dir", help="Directory to write config files and variables")
...
left_context = 0
right_context = 0
split1 = args.splice_indexes.split()
# we already checked the string is nonempty.
if len(split1) < 1:
    sys.exit("invalid --splice-indexes argument, too short: " + args.splice_indexes)
try:
    for string in split1:
        ...
        if not int_list == sorted(int_list):
            sys.exit(
                "elements of --splice-indexes must be sorted: " + args.splice_indexes
            )
...
list = [
    ("Offset(input, {0})".format(n) if n != 0 else "input") for n in splice_array[0]
]
...
for l in range(1, num_hidden_layers + 1):
    ...
    if l == 1:
        print(
            "component name=lda type=FixedAffineComponent matrix={0}/lda.mat".format(
                args.config_dir
            ),
            file=f,
        )
    ...
    print(
        "# Note: param-stddev in next component defaults to 1/sqrt(input-dim).", file=f
    )
    print(
        "component name=affine{0} type=NaturalGradientAffineComponent "
        "input-dim={1} output-dim={2} bias-stddev=0".format(
            l, cur_dim, nonlin_input_dim
        ),
        file=f,
    )
    if args.relu_dim is not None:
        print(
            "component name=nonlin{0} type=RectifiedLinearComponent dim={1}".format(
                l, args.relu_dim
            ),
            file=f,
        )
        ...
        print(
            "component name=nonlin{0} type=PnormComponent input-dim={1} output-dim={2}".format(
                l, args.pnorm_input_dim, args.pnorm_output_dim
            ),
            file=f,
        )
    print(
        "component name=renorm{0} type=NormalizeComponent dim={1} target-rms={2}".format(
            l,
            nonlin_output_dim,
            (1.0 if l < num_hidden_layers else args.final_layer_normalize_target),
        ),
        file=f,
    )
    print(
        "component name=final-affine type=NaturalGradientAffineComponent "
    ...
    if args.include_log_softmax == "true":
        if use_presoftmax_prior_scale:
            print(
                "component name=final-fixed-scale type=FixedScaleComponent "
                "scales={0}/presoftmax_prior_scale.vec".format(args.config_dir),
                file=f,
            )
        print(
            "component name=final-log-softmax type=LogSoftmaxComponent dim={0}".format(
                args.num_targets
            ),
            file=f,
        )
    ...
    if l == 1:
        splices = [
            ("Offset(input, {0})".format(n) if n != 0 else "input")
        ...
        print(
            "component-node name=lda component=lda input={0}".format(orig_input), file=f
        )
        cur_input = "lda"
    else:
        ...
        splices = [
            (
                "Offset(renorm{0}, {1})".format(l - 1, n)
                if n != 0
                else "renorm{0}".format(l - 1)
            )
            for n in splice_array[l - 1]
        ]
        cur_input = "Append({0})".format(", ".join(splices))
    print(
        "component-node name=affine{0} component=affine{0} input={1} ".format(
            l, cur_input
        ),
        file=f,
    )
    print(
        "component-node name=nonlin{0} component=nonlin{0} input=affine{0}".format(l),
        file=f,
    )
    print(
        "component-node name=renorm{0} component=renorm{0} input=nonlin{0}".format(l),
        file=f,
    )

    print(
        "component-node name=final-affine component=final-affine input=renorm{0}".format(
            l
        ),
        file=f,
    )
    ...
    if args.include_log_softmax == "true":
        if use_presoftmax_prior_scale:
            ...
            print(
                "component-node name=final-log-softmax component=final-log-softmax "
                "input=final-fixed-scale",
                file=f,
            )
        else:
            print(
