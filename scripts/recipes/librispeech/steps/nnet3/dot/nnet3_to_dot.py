...
node_attributes = {
    "input-node": {"shape": "oval"},
    "output-node": {"shape": "oval"},
    ...
    "ConvolutionComponent": {"color": "lightpink", "shape": "box", "style": "filled"},
    "FixedScaleComponent": {"color": "blueviolet", "shape": "box", "style": "filled"},
    "FixedAffineComponent": {
        "color": "darkolivegreen1",
        "shape": "box",
        "style": "filled",
    },
    "SigmoidComponent": {"color": "bisque", "shape": "rectangle", "style": "filled"},
    "TanhComponent": {"color": "bisque", "shape": "rectangle", "style": "filled"},
    "NormalizeComponent": {
        "color": "aquamarine",
        "shape": "rectangle",
        "style": "filled",
    ...
    "LogSoftmaxComponent": {"color": "cyan", "shape": "rectangle", "style": "filled"},
}
...
def ProcessAppendDescriptor(segment, parent_node_name, affix, edge_attributes=None):
    ...
    for i in range(len(segment["arguments"])):
        ...
        names.append(
            "<{0}> part {1}".format(GetDotNodeName(part_name)["node"], part_index + i)
        )
        dot_graph.append(
            "{0} -> {1}:{2}".format(
    ...
    label = "{{" + label + "}|Append}"
    dot_graph.append(
        '{0} [shape=Mrecord, label="{1}"];'.format(
    ...
    dot_string = "{0} -> {1} [tailport=s]".format(
        GetDotNodeName(desc_name)["node"], GetDotNodeName(parent_node_name)["node"]
    )
...
def ProcessSumDescriptor(segment, parent_node_name, affix, edge_attributes=None):
    ...
    for i in range(len(segment["sub_segments"])):
        ...
        dot_graph += DescriptorSegmentToDot(
            sub_segment,
            "{0}:{1}".format(desc_name, part_name),
    ...
    label = "{{" + label + "}|Sum}"
    dot_graph.append(
        '{0} [shape=Mrecord, label="{1}", color=red];'.format(
            GetDotNodeName(desc_name)["node"], label
        )
    )
...
def ProcessReplaceIndexDescriptor(
...
def ProcessIfDefinedDescriptor(segment, parent_node_name, affix, edge_attributes=None):
    ...
    if segment["arguments"]:
        dot_graph.append(
            '{0} -> {1} [style=dotted, label="IfDefined"]'.format(
                GetDotNodeName(segment["arguments"][0])["node"],
                GetDotNodeName(parent_node_name)["node"],
            )
        )
...
def DescriptorSegmentToDot(segment, parent_node_name, affix, edge_attributes=None):
    ...
    if segment["name"] == "Append":
        dot_graph += ProcessAppendDescriptor(
            segment, parent_node_name, affix, edge_attributes
        )
    elif segment["name"] == "Offset":
        dot_graph += ProcessOffsetDescriptor(
            segment, parent_node_name, affix, edge_attributes
        )
    elif segment["name"] == "Sum":
        dot_graph += ProcessSumDescriptor(
            segment, parent_node_name, affix, edge_attributes
        )
        ...
        dot_graph += ProcessRoundDescriptor(
            segment, parent_node_name, affix, edge_attributes
        )
        ...
        raise Exception(
            "Descriptor {0}, is not recognized by this script. Please add Process{0}Descriptor method".format(
                segment["name"]
            )
        )
    return dot_graph
...
def Nnet3DescriptorToDot(descriptor, parent_node_name):
    ...
    if segments:
        for segment in segments:
            dot_lines += DescriptorSegmentToDot(
                segment, parent_node_name, parent_node_name
            )
    elif arguments:
...
def Nnet3ComponentToDot(component_config, component_attributes=None):
    ...
    attributes_to_print = set(component_attributes).intersection(
        list(component_config.keys())
    )
    ...
    return [
        '{0} [label="{1}" {2}]'.format(
            GetDotNodeName(component_config["name"], is_component=True)["node"],
            label,
            attr_string,
        )
    ]
...
def Nnet3DimrangeToDot(parsed_config):
    ...
    dot_graph.append(
        '{0} [shape=rectangle, label="{1}"]'.format(dot_node["node"], dot_node["label"])
    )
    dot_graph.append(
        '{0} -> {1} [taillabel="dimrange({2}, {3})"]'.format(
...
def Nnet3ComponentNodeToDot(parsed_config):
    ...
    dot_node = GetDotNodeName(parsed_config["name"])
    dot_graph.append(
        '{0} [ label="{1}", shape=box ]'.format(dot_node["node"], dot_node["label"])
    )
    dot_graph.append(
        "{0} -> {1} [ weight=10 ]".format(
