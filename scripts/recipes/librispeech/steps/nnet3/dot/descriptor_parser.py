...
def ParseSubsegmentsAndArguments(
    segment_endpoints, sub_segments, arguments, input_string
):
    ...
    if len(sub_segments) > 0:
        ...
        arguments = [
            ...
            for x in input_string[
                segment_endpoints[0] : segment_endpoints[1] + 1
            ].split()
        ]
        sub_segments = []
...
def IdentifyNestedSegments(input_string):
    ...
    for i in range(len(input_string)):
        ...
        if input_string[i] == end_identifier:
            ...
            sub_segments, arguments = ParseSubsegmentsAndArguments(
                [current_segment_endpoints[0] + 1, current_segment_endpoints[1] - 1],
                sub_segments,
                arguments,
                input_string,
            )
    ...
    if arguments:
        if segments:
            raise Exception(
                "Arguments not expected outside top level braces : {0}".format(
                    input_string
                )
            )
    if len(segments) > 1:
