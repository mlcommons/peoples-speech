...
def GetArgs():
    ...
    feat_group.add_argument(
        "--feat-dim", type=int, help="Raw feature dimension, e.g. 13"
    )
    feat_group.add_argument(
        "--feat-dir",
        type=str,
    ...
    ivector_group.add_argument(
        "--ivector-dim", type=int, help="iVector dimension, e.g. 100", default=0
    )
    ivector_group.add_argument(
        "--ivector-dir",
        type=str,
        help="iVector dir, which will be used to derive the ivector-dim  ",
        default=None,
    )

    num_target_group = parser.add_mutually_exclusive_group(required=True)
    num_target_group.add_argument(
        "--num-targets",
        type=int,
    ...
    parser.add_argument(
        ...
        "when CNN layers are used, no LDA will be added",
        default=None,
    )
    parser.add_argument(
        "--cnn.bottleneck-dim",
        type=int,
        dest="cnn_bottleneck_dim",
        help="Output dimension of the linear layer at the CNN output "
        "for dimension reduction, e.g. 256."
        "The default zero means this layer is not needed.",
        default=0,
    )
    parser.add_argument(
        "--cnn.cepstral-lifter",
        type=float,
        dest="cepstral_lifter",
    ...
    parser.add_argument(
        "--final-layer-normalize-target",
        type=float,
        help="RMS target for final layer (set to <1 if final layer learns too fast",
        default=1.0,
    )
    parser.add_argument(
        "--max-change-per-component",
        type=float,
        help="Enforces per-component max change (except for the final affine layer). "
        "if 0 it would not be enforced.",
        default=0.75,
    )
    parser.add_argument(
        "--max-change-per-component-final",
        type=float,
    ...
    parser.add_argument(
        "--pnorm-input-dim", type=int, help="input dimension to p-norm nonlinearities"
    )
    parser.add_argument(
        "--pnorm-output-dim", type=int, help="output dimension of p-norm nonlinearities"
    )
    relu_dim_group = parser.add_mutually_exclusive_group(required=False)
    relu_dim_group.add_argument(
        "--relu-dim", type=int, help="dimension of all ReLU nonlinearity layers"
    )
    relu_dim_group.add_argument(
        "--relu-dim-final",
        type=int,
        help="dimension of the last ReLU nonlinearity layer. Dimensions increase geometrically from the first through the last ReLU layer.",
        default=None,
    )
    parser.add_argument(
        "--relu-dim-init",
        type=int,
        help="dimension of the first ReLU nonlinearity layer. Dimensions increase geometrically from the first through the last ReLU layer.",
        default=None,
    )

    parser.add_argument(
        "--self-repair-scale-nonlinearity",
        type=float,
        help="A non-zero value activates the self-repair mechanism in the sigmoid and tanh non-linearities of the LSTM",
        default=None,
    )

    parser.add_argument(
        "--use-presoftmax-prior-scale",
        type=str,
        action=common_lib.StrToBoolAction,
    ...
    parser.add_argument(
        "config_dir", help="Directory to write config files and variables"
    )
...
def CheckArgs(args):
    ...
    if not args.relu_dim is None:
        if (
            not args.pnorm_input_dim is None
            or not args.pnorm_output_dim is None
            or not args.relu_dim_init is None
        ):
            raise Exception(
        ...
        if not args.pnorm_input_dim is None or not args.pnorm_output_dim is None:
            raise Exception(
                "--relu-dim-final argument not compatible with "
        ...
        if (args.nonlin_input_dim < args.nonlin_output_dim) or (
            ...
            raise Exception(
                "Invalid --pnorm-input-dim {0} and --pnorm-output-dim {1}".format(
                    args.nonlin_input_dim, args.nonlin_output_dim
                )
            )
        args.nonlin_output_dim_final = None
        args.nonlin_output_dim_init = None
...
def AddConvMaxpLayer(config_lines, name, input, args):
    ...
    input = nodes.AddConvolutionLayer(
        config_lines,
        name,
        input,
        input["3d-dim"][0],
    ...
    if args.pool_x_size > 1 or args.pool_y_size > 1 or args.pool_z_size > 1:
        input = nodes.AddMaxpoolingLayer(
            config_lines,
            name,
            input,
            input["3d-dim"][0],
...
def AddCnnLayers(
    ...
    common_lib.write_idct_matrix(
        feat_dim, cepstral_lifter, config_dir.strip() + "/idct.mat"
    )
    prev_layer_output = {"descriptor": "input", "dimension": feat_dim}
    prev_layer_output = nodes.AddFixedAffineLayer(
        config_lines, "Idct", prev_layer_output, config_dir.strip() + "/idct.mat"
    )

    list = [
        (
            "Offset({0}, {1})".format(prev_layer_output["descriptor"], n)
            if n != 0
            else prev_layer_output["descriptor"]
        )
    ...
    for cl in range(0, num_cnn_layers):
        prev_layer_output = AddConvMaxpLayer(
            config_lines, "L{0}".format(cl), prev_layer_output, cnn_args[cl]
        )

    if cnn_bottleneck_dim > 0:
        prev_layer_output = nodes.AddAffineLayer(
            config_lines, "cnn-bottleneck", prev_layer_output, cnn_bottleneck_dim, ""
        )

    if ivector_dim > 0:
        iv_layer_output = {
            "descriptor": "ReplaceIndex(ivector, t, 0)",
            "dimension": ivector_dim,
        }
        iv_layer_output = nodes.AddAffineLayer(
            config_lines, "ivector", iv_layer_output, ivector_dim, ""
        )
        prev_layer_output["descriptor"] = "Append({0}, {1})".format(
            prev_layer_output["descriptor"], iv_layer_output["descriptor"]
        )
        prev_layer_output["dimension"] = (
            prev_layer_output["dimension"] + iv_layer_output["dimension"]
        )
...
def ParseSpliceString(splice_indexes):
    ...
    if len(split1) < 1:
        raise Exception("invalid splice-indexes argument, too short: " + splice_indexes)
    try:
        for string in split1:
            split2 = string.split(",")
            if len(split2) < 1:
                raise Exception(
                    "invalid splice-indexes argument, too-short element: "
                    + splice_indexes
                )
            int_list = []
            ...
            if not int_list == sorted(int_list):
                raise Exception(
                    "elements of splice-indexes must be sorted: " + splice_indexes
                )
...
def MakeConfigs(
    ...
    nonlin_output_dim,
    subset_dim,
    nonlin_output_dim_init,
    nonlin_output_dim_final,
    ...
    if xent_separate_forward_affine:
        if splice_indexes[-1] != [0]:
            raise Exception(
                "--xent-separate-forward-affine option is supported only if the last-hidden layer has no splicing before it. Please use a splice-indexes with just 0 as the final splicing config."
            )
    ...
    if cnn_layer is not None:
        prev_layer_output = AddCnnLayers(
            config_lines,
            cnn_layer,
            cnn_bottleneck_dim,
            cepstral_lifter,
    ...
    if not nonlin_output_dim is None:
        ...
        raise Exception(
            "num-hidden-layers has to be greater than 1 if relu-dim-init and relu-dim-final is different."
        )
    else:
        # computes relu-dim for each hidden layer. They increase geometrically across layers
        factor = (
            pow(
                float(nonlin_output_dim_final) / nonlin_output_dim_init,
                1.0 / (num_hidden_layers - 1),
            )
            if num_hidden_layers > 1
            else 1
        )
        nonlin_output_dims = [
            int(round(nonlin_output_dim_init * pow(factor, i)))
            for i in range(0, num_hidden_layers)
        ]
        assert (
            nonlin_output_dims[-1] >= nonlin_output_dim_final - 1
            and nonlin_output_dims[-1] <= nonlin_output_dim_final + 1
        )  # due to rounding error
        nonlin_output_dims[
            -1
        ] = nonlin_output_dim_final  # It ensures that the dim of the last hidden layer is exactly the same as what is specified
    ...
    for i in range(0, num_hidden_layers):
        ...
        if not (len(splice_indexes[i]) == 1 and splice_indexes[i][0] == 0):
            ...
            if subset_dim > 0:
                ...
                subset_node_config = "dim-range-node name=Tdnn_input_{0} input-node={1} dim-offset={2} dim={3}".format(
                    i, prev_layer_output_descriptor, 0, subset_dim
                )
                subset_output = {
                    "descriptor": "Tdnn_input_{0}".format(i),
            ...
            prev_layer_output = {
                "descriptor": "Append({0})".format(" , ".join(appended_descriptors)),
        ...
        if xent_separate_forward_affine and i == num_hidden_layers - 1:
            if xent_regularize == 0.0:
                raise Exception(
                    "xent-separate-forward-affine=True is valid only if xent-regularize is non-zero"
                )

            if nonlin_type == "relu":
                prev_layer_output_chain = nodes.AddAffRelNormLayer(
                    config_lines,
                    "Tdnn_pre_final_chain",
                    prev_layer_output,
                    nonlin_output_dim,
                ...
                prev_layer_output_xent = nodes.AddAffRelNormLayer(
                    config_lines,
                    "Tdnn_pre_final_xent",
                    prev_layer_output,
                    nonlin_output_dim,
                ...
                prev_layer_output_chain = nodes.AddAffPnormLayer(
                    config_lines,
                    "Tdnn_pre_final_chain",
                    prev_layer_output,
                    nonlin_input_dim,
                ...
                prev_layer_output_xent = nodes.AddAffPnormLayer(
                    config_lines,
                    "Tdnn_pre_final_xent",
                    prev_layer_output,
                    nonlin_input_dim,
            ...
            if nonlin_type == "relu":
                prev_layer_output = nodes.AddAffRelNormLayer(
                    config_lines,
                    "Tdnn_{0}".format(i),
                    prev_layer_output,
                    ...
                    else final_layer_normalize_target,
                    self_repair_scale=self_repair_scale,
                    max_change_per_component=max_change_per_component,
                )
            elif nonlin_type == "pnorm":
                prev_layer_output = nodes.AddAffPnormLayer(
                    config_lines,
                    "Tdnn_{0}".format(i),
                    prev_layer_output,
                    nonlin_input_dim,
