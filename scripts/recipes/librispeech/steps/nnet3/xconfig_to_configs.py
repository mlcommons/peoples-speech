...
def get_args():
    ...
    parser.add_argument(
        "--xconfig-file", required=True, help="Filename of input xconfig file"
    )
    parser.add_argument(
        "--existing-model",
...
def backup_xconfig_file(xconfig_file, config_dir):
    ...
    try:
        ...
        raise Exception(
            ...
            "{1}/xconfig for output".format(sys.argv[0], config_dir)
        )
    try:
        xconfig_file_in = open(xconfig_file)
    except:
        raise Exception(
            "{0}: error opening file {1} for input" "".format(sys.argv[0], config_dir)
        )
...
def write_expanded_xconfig_files(config_dir, all_layers):
    ...
    try:
        ...
        raise Exception(
            "{0}: error opening file "
            "{1}/xconfig.expanded.1 for output".format(sys.argv[0], config_dir)
        )
    ...
    try:
        ...
        raise Exception(
            "{0}: error opening file "
            "{1}/xconfig.expanded.2 for output".format(sys.argv[0], config_dir)
        )
...
def write_config_files(config_dir, all_layers):
    ...
    for layer in all_layers:
        try:
            ...
            print(
                "{0}: error producing config lines from xconfig "
                "line '{1}': error was: {2}".format(sys.argv[0], str(layer), repr(e)),
                file=sys.stderr,
            )
    ...
    for basename, lines in config_basename_to_lines.items():
        ...
        num_output_node_lines = sum(
            [1 if line.startswith("output-node") else 0 for line in lines]
        )
        if num_output_node_lines == 0:
            if basename == "init":
                ...
                print(
                    "{0}: error in xconfig file {1}: may be lack of a "
        ...
        try:
            ...
            print(
                "{0}: error writing to config file {1}: error is {2}"
                "".format(sys.argv[0], filename, repr(e)),
                file=sys.stderr,
            )
            # we use raise rather than raise(e) as using a blank raise
            # preserves the backtrace
...
def add_nnet_context_info(config_dir, nnet_edits=None, existing_model=None):
    ...
    common_lib.execute_command(
        ...
        "{1}/ref.raw"
        "".format(existing_model if existing_model is not None else "", config_dir)
    )
...
def check_model_contexts(config_dir, nnet_edits=None, existing_model=None):
    ...
    if "init" in contexts:
        ...
        if "left-context" in contexts["init"] and "left-context" in contexts["ref"]:
            if (contexts["init"]["left-context"] > contexts["ref"]["left-context"]) or (
                contexts["init"]["right-context"] > contexts["ref"]["right-context"]
            ):
                raise Exception(
...
def main():
    ...
    check_model_contexts(
        args.config_dir, args.nnet_edits, existing_model=args.existing_model
    )
    add_nnet_context_info(
        args.config_dir, args.nnet_edits, existing_model=args.existing_model
    )
