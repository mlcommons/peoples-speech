...
def GetArgs():
    ...
    parser.add_argument(
        "--splice-indexes",
        type=str,
        help="Splice indexes at input layer, e.g. '-3,-2,-1,0,1,2,3'",
        required=True,
        default="0",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-lstm-layers",
        type=int,
        help="Number of LSTM layers to be stacked",
        default=1,
    )
    parser.add_argument("--cell-dim", type=int, help="dimension of lstm-cell")
    parser.add_argument(
        "--recurrent-projection-dim", type=int, help="dimension of recurrent projection"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--hidden-dim", type=int, help="dimension of fully-connected layers"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="options to be supplied to NaturalGradientPerElementScaleComponent",
        default="",
    )
    parser.add_argument(
        "--ng-affine-options",
        type=str,
        help="options to be supplied to NaturalGradientAffineComponent",
        default="",
    )
    ...
    parser.add_argument(
        ...
        "if clipping-threshold=0 no clipping is done",
        default=30,
    )
    parser.add_argument(
        "--zeroing-threshold",
        ...
        "if zeroing-threshold=0 no periodic zeroing is done",
        default=15.0,
    )
    parser.add_argument(
        "--zeroing-interval",
        type=int,
        help="zeroing interval used in BackpropTruncation components",
        default=20,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--self-repair-scale-clipgradient",
        type=float,
        help="Outdated option retained for back compatibility, has no effect.",
        default=1.0,
    )
...
def ParseLstmDelayString(lstm_delay):
    ...
    try:
        for i in range(len(split1)):
            indexes = [
                int(x)
                for x in split1[i].strip().lstrip("[").rstrip("]").strip().split(",")
            ]
            if len(indexes) < 1:
                raise ValueError(
                    "invalid --lstm-delay argument, too-short element: " + lstm_delay
                )
            elif len(indexes) == 2 and indexes[0] * indexes[1] >= 0:
                raise ValueError(
                    "Warning: {} is not a standard BLSTM mode. There should be a negative delay for the forward, and a postive delay for the backward.".format(
                        indexes
                    )
                )
            if (
                len(indexes) == 2 and indexes[0] > 0
...
def MakeConfigs(
    ...
    for i in range(num_lstm_layers):
        if len(lstm_delay[i]) == 2:  # add a bi-directional LSTM layer
            prev_layer_output = nodes.AddBLstmLayer(
            ...
            prev_layer_output = nodes.AddLstmLayer(
                config_lines=config_lines,
