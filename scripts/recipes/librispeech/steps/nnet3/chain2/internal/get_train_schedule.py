    ...
    with open(args.schedule_out, "w", encoding="latin-1") as ostream:
        for iter in range(num_iters):
            ...
            dropout_edit_option = common_train_lib.get_dropout_edit_option(
                ...
                float(num_scp_files_processed)
                / max(1, (num_scp_files_to_process - args.num_jobs_final)),
                iter,
            )
            ...
            for job in range(1, current_num_jobs + 1):
                ...
                frame_shift = (
                    scp_index + k // args.num_scp_files
            ...
            print(
                """iter={iter}; num_jobs={nj}; inv_num_jobs={nj_inv}; scp_indexes=(pad {indexes}); frame_shifts=(pad {shifts}); dropout_opt="{opt}"; lrate={lrate}""".format(
                    iter=iter,
                    nj=current_num_jobs,
                    nj_inv=(1.0 / current_num_jobs),
                    indexes=" ".join(egs),
                    shifts=" ".join(frame_shifts),
                    opt=dropout_edit_option,
                    lrate=lrate,
                ),
                file=ostream,
            )
