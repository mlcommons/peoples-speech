...
flags.DEFINE_bool(
    "use_fp16", False, "Train using 16-bit floats instead of 32bit floats"
)
...
class RnnlmModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        def rnn_cell():
            ...
            if "reuse" in inspect.getargspec(tf.contrib.rnn.BasicRNNCell.__init__).args:
        ...
        self.initial = tf.reshape(
            tf.stack(axis=0, values=self._initial_state_single),
        ...
        test_state_out = tf.reshape(
            tf.stack(axis=0, values=test_output_state),
            [config.num_layers, 1, size],
            name="test_state_out",
        )
