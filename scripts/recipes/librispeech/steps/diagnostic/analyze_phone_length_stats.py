...
try:
    ...
    sys.exit(
        "analyze_phone_length_stats.py: error opening or reading {0}/phones.txt".format(
            args.lang
        )
    )
...
try:
    ...
    sys.exit(
        "analyze_phone_length_stats.py: error processing {0}/phones/silence.csl: {1}".format(
            args.lang, str(e)
        )
    )
...
while True:
    ...
    try:
        ...
        sys.exit(
            "analyze_phone_length_stats.py: unexpected phone {0} "
...
total_frames["internal"] = (
    total_frames["all"] - total_frames["begin"] - total_frames["end"]
)
total_phones["internal"] = (
    total_phones["all"] - total_phones["begin"] - total_phones["end"]
)
...
if total_phones["internal"] != 0.0:
    ...
    total_optsil_frames = sum(
        [float(l * c) for l, c in phone_lengths["all"][optional_silence_phone].items()]
    )
    opt_sil_internal_frame_percent = (
        total_internal_optsil_frames * 100.0 / total_frames["internal"]
    )
    ...
    opt_sil_internal_phone_percent = (
        sum(internal_opt_sil_phone_lengths.values()) * 100.0 / total_phones["internal"]
    )
