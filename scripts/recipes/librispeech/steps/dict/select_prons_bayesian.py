    ...
    for word, entry in posteriors.items():
        ...
        for pron, count in entry:
            ...
            if word in g2p_lexicon and pron in g2p_lexicon[word]:
                ...
                word in phonetic_decoding_lexicon
                and pron in phonetic_decoding_lexicon[word]
            ):
                source = "P"
...
def SelectPronsBayesian(
    args, counts, posteriors, ref_lexicon, g2p_lexicon, phonetic_decoding_lexicon
):
    ...
    for word, entry in posteriors.items():
        ...
        last_post = 0.0
        while (num_variants < variants_counts and post_tot < variants_prob_mass) or (
            len(entry) > 0 and entry[-1][1] == last_post
        ):  # this conditions
    ...
    print(
        "Num. words in the learned lexicon: {0} num. selected prons: {1}".format(
            len(learned_lexicon), num_prons_tot
        ),
        file=sys.stderr,
    )
    print(
        "{0} selected prons came from reference candidate prons; {1} came from G2P candidate prons;"
        "{2} came from phonetic-decoding candidate prons.".format(
            reference_selected, g2p_selected, phonetic_decoding_selected
        ),
        file=sys.stderr,
    )
...
def WriteEditsAndSummary(
    ...
    src = {}
    print(
        "# Note: This file contains pronunciation info for words who have candidate"
    ...
    print(
        "# 1st Col: source of the candidate pron: G(2P) / P(hone-decoding) / R(eference).",
        file=args.ref_lexicon_edits_handle,
    )
    print(
        "# 2nd Col: accepted or not in the learned lexicon (Y/N).",
        file=args.ref_lexicon_edits_handle,
    )
    print(
        "# 3rd Col: soft counts from lattice-alignment (not augmented by prior-counts).",
        file=args.ref_lexicon_edits_handle,
    )
    ...
    words_to_edit_sorted = sorted(
        words_to_edit, key=lambda entry: entry[1], reverse=True
    )
    for word, count in words_to_edit_sorted:
        ...
        for pron in ref_lexicon[word]:
            if pron not in learned_lexicon[word]:
                ...
                print(
                    "R  | N |  {:.2f} | {} ".format(soft_count, pron),
                    file=args.ref_lexicon_edits_handle,
                )
    print(
        "Here are the words whose reference pron candidates were all declined",
        words[0]["100"],
        file=sys.stderr,
    )
    print(
    ...
    print(
        "In the learned lexicon, out of those",
        len(ref_lexicon),
        "words from the vocab of the reference lexicon:",
        file=sys.stderr,
    )
    print(
        "  For those frequent words whose counts in the training text > ",
        threshold,
        ":",
        file=sys.stderr,
    )
    num_freq_ivs_from_all_sources = (
        len(words[0]["111"]) + len(words[0]["110"]) + len(words[0]["011"])
    )
    num_freq_ivs_from_g2p_or_phonetic_decoding = (
        len(words[0]["101"]) + len(words[0]["001"]) + len(words[0]["100"])
    )
    num_freq_ivs_from_ref = len(words[0]["010"])
    num_infreq_ivs_from_all_sources = (
        len(words[1]["111"]) + len(words[1]["110"]) + len(words[1]["011"])
    )
    num_infreq_ivs_from_g2p_or_phonetic_decoding = (
        len(words[1]["101"]) + len(words[1]["001"]) + len(words[1]["100"])
    )
    num_infreq_ivs_from_ref = len(words[1]["010"])
    print(
        " {} words' selected prons came from the reference lexicon, G2P/phonetic-decoding.".format(
            num_freq_ivs_from_all_sources
        ),
        file=sys.stderr,
    )
    print(
        " {} words' selected prons come from G2P/phonetic-decoding-generated.".format(
            num_freq_ivs_from_g2p_or_phonetic_decoding
        ),
        file=sys.stderr,
    )
    print(
        " {} words' selected prons came from the reference lexicon only.".format(
            num_freq_ivs_from_ref
        ),
        file=sys.stderr,
    )
    print(
        "  For those words whose counts in the training text <= {}:".format(threshold),
        file=sys.stderr,
    )
    print(
        " {} words' selected prons came from the reference lexicon, G2P/phonetic-decoding.".format(
    ...
    print(
        "  In the learned lexicon, out of those {} OOV words (w.r.t the reference lexicon):".format(
            num_oovs
        ),
        file=sys.stderr,
    )
    ...
    print(
        "    {} words' selected prons came from G2P and phonetic-decoding.".format(
            num_freq_oovs_from_both_sources
        ),
        file=sys.stderr,
    )
    print(
        "    {} words' selected prons came from phonetic decoding only.".format(
            num_freq_oovs_from_phonetic_decoding
        ),
        file=sys.stderr,
    )
    print(
        "    {} words' selected prons came from G2P only.".format(
            num_freq_oovs_from_g2p
        ),
        file=sys.stderr,
    )
    print(
    ...
    print(
        "    {} words' selected prons came from G2P and phonetic-decoding.".format(
            num_infreq_oovs_from_both_sources
        ),
        file=sys.stderr,
    )
    print(
        "    {} words' selected prons came from phonetic decoding only.".format(
            num_infreq_oovs_from_phonetic_decoding
        ),
        file=sys.stderr,
    )
    print(
        "    {} words' selected prons came from G2P only.".format(
            num_infreq_oovs_from_g2p
        ),
        file=sys.stderr,
    )
...
def Main():
    ...
    phonetic_decoding_lexicon = ReadLexicon(
        args, args.phonetic_decoding_lexicon_handle, counts
    )
    stats = ReadPronStats(args.pron_stats_file_handle)
    phonetic_decoding_lexicon, stats = FilterPhoneticDecodingLexicon(
        args, phonetic_decoding_lexicon, stats
    )

    # Compute prior counts
    prior_counts = ComputePriorCounts(
        args, counts, ref_lexicon, g2p_lexicon, phonetic_decoding_lexicon
    )
