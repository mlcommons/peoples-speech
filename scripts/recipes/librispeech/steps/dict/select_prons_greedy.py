...
def GetArgs():
    ...
    parser.add_argument(
        "pd_lexicon",
        metavar="<phonetic-decoding-lexicon>",
        type=str,
...
def CheckArgs(args):
    ...
    for i in range(0, 3):
        if float(alpha[i]) < 0 or float(alpha[i]) > 1:
            raise Exception(
                "alaph ", alpha[i], " is invalid, it must be within [0, 1]."
            )
        if float(alpha[i]) == 0:
            alpha[i] = -1e-3
    ...
    for i in range(0, 3):
        if float(beta[i]) < 0 or float(beta[i]) > 100:
            raise Exception(
                "beta ", beta[i], " is invalid, it must be within [0, 100]."
            )
    ...
    if args.delta <= 0 or args.delta > 0.1:
        raise Exception(
            "delta ", args.delta, " is invalid, it must be within" "(0, 0.01)."
        )
...
def SelectPronsGreedy(
    args, stats, counts, ref_lexicon, g2p_lexicon, pd_lexicon, dianostic_info=False
):
    prons = defaultdict(
        list
    )  # Put all possible prons from three source lexicons into this dictionary
    src = (
        {}
    )  # Source of each (word, pron) pair: 'P' = phonetic-decoding, 'G' = G2P, 'R' = reference
    ...
    for word in prons:
        ...
        while len(active_indexes) > 1:
            ...
            while abs(log_like - log_like_last) > 1e-7:
                ...
                pron_probs, log_like = OneEMIter(
                    args, word, stats, prons, pron_probs, False
                )
                if (
                    log_like_last == 1.0 and len(soft_counts_normalized) == 0
                ):  # the first iteration
                    soft_counts_normalized = pron_probs
                    if dianostic_info:
                        print(
                            "Avg.(over all egs) soft counts: {}".format(
                                soft_counts_normalized
                            )
                        )
            if dianostic_info:
            ...
            for i in active_indexes:
                ...
                while abs(log_like2 - log_like2_last) > 0.001:
                    ...
                    pron_probs_mod, log_like2 = OneEMIter(
                        args, word, stats, prons, pron_probs_mod, False
                    )

                loss_abs = (
                    log_like - log_like2
                )  # absolute likelihood loss before normalization
                ...
                if source == "P":
                    thr *= args.alpha[0]
                    loss *= float(len(stats[word])) / (
                        float(len(stats[word])) + args.beta[0]
                    )
                if source == "G":
                    thr *= args.alpha[1]
                    loss *= float(len(stats[word])) / (
                        float(len(stats[word])) + args.beta[1]
                    )
                if source == "R":
                    thr *= args.alpha[2]
                    loss *= float(len(stats[word])) / (
                        float(len(stats[word])) + args.beta[2]
                    )
                if loss - thr < 0:  # loss - thr here is just q_b
                    if dianostic_info:
                        print(
                            "Smoothed log-like loss {} is smaller than threshold {} so that the quality"
            ...
            if len(candidates_to_delete) == 0:
                break
            candidates_to_delete_sorted = sorted(
                candidates_to_delete,
            ...
            if dianostic_info:
                print(
                    "removed pron {}, from source {} with quality score {:.5f}".format(
                        pron, source, quality_score
                    )
                )
                if source == "P" and soft_count > 0.7 and len(stats[word]) > 5:
                    print(
                        "WARNING: alpha_{pd} or beta_{pd} may be too large!"
                        ...
                        "    rejected if alpha_{pd} is smaller than {}".format(
                            ...
                            soft_count,
                            -loss / log_delta,
                            -args.alpha[0] * len(stats[word])
                            + (objf_change + args.beta[0]),
                        ),
                        file=sys.stderr,
                    )
                    if loss_abs > thr:
                        print(
                            "    or beta_{pd} is smaller than {}".format(
                                (loss_abs / thr - 1) * len(stats[word])
                            ),
                            file=sys.stderr,
                        )
                if source == "G" and soft_count > 0.7 and len(stats[word]) > 5:
                    print(
                        "WARNING: alpha_{g2p} or beta_{g2p} may be too large!"
                        "    For the word '{}' whose count is {}, the candidate "
                        ...
                        "    rejected if alpha_{g2p} is smaller than {} ".format(
                            ...
                            soft_count,
                            -loss / log_delta,
                            -args.alpha[1] * len(stats[word])
                            + (objf_change + args.beta[1]),
                        ),
                        file=sys.stderr,
                    )
                    if loss_abs > thr:
                        print(
                            "    or beta_{g2p} is smaller than {}.".format(
                                (loss_abs / thr - 1) * len(stats[word])
                            ),
                            file=sys.stderr,
                        )
...
def Main():
    ...
    learned_lexicon = SelectPronsGreedy(
        args, stats, counts, ref_lexicon, g2p_lexicon, pd_lexicon
    )
