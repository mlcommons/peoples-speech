...
def GetArgs():
    parser = argparse.ArgumentParser(
        description="Converts pronunciation statistics (from phonetic decoding or g2p) "
    ...
    parser.add_argument(
        ...
        choices=["true", "false"],
        help="If normalize lexicon such that the sum of " "probabilities is 1.",
    )
    parser.add_argument(
        "--set-max-to-one",
        type=str,
        ...
        action=StrToBoolAction,
        choices=["true", "false"],
        help="If normalize lexicon such that the max " "probability is 1.",
    )
    parser.add_argument(
        "--top-N",
        type=int,
    ...
    parser.add_argument(
        "out_lexicon", metavar="<out-lexicon>", type=str, help="Output lexicon."
    )
...
def CheckArgs(args):
    ...
    if args.set_max_to_one == args.set_sum_to_one:
        raise Exception(
            "Cannot have both " "set-max-to-one and set-sum-to-one as true or false."
        )
...
def NormalizeLexicon(lexicon, set_max_to_one=True, set_sum_to_one=False, min_prob=0):
    word_probs = {}
...
def WriteLexicon(args, lexicon, filter_lexicon):
    ...
    for entry, prob in lexicon.iteritems():
        ...
        if entry in filter_lexicon:
            ...
            continue
        words.add(entry[0])
        print("{0} {1}".format(entry[0], entry[1]), file=args.out_lexicon_handle)
    print(
        "Before pruning, the total num. pronunciations is: {}".format(len(lexicon)),
        file=sys.stderr,
    )
    print(
    ...
    print(
        "Filtered out {} pronunciations in the filter lexicon.".format(num_filtered),
        file=sys.stderr,
    )
