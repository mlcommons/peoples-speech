...
def GetArgs():
    ...
    parser.add_argument(
        "learned_lexicon",
        metavar="<learned-lexicon>",
        type=str,
        help="Learned lexicon." "Each line must be <word> <phones>",
    )
    parser.add_argument(
        "learned_lexicon_oov",
...
def ReadArcStats(arc_stats_file_handle):
    ...
    for line in arc_stats_file_handle.readlines():
        ...
        if len(splits) < 5:
            raise Exception("Invalid format of line " + line + " in " + arc_stats_file)
        utt = splits[1]
...
def WriteEditsAndSummary(
    ...
    for word, count in words_to_edit_sorted:
        ...
        learned_prons = []
        for pron in learned_lexicon[word]:
            learned_prons.append(
                (src[(word, pron)], "Y", stats_summed[(word, pron)], pron)
            )
        for pron in ref_lexicon[word]:
            if pron not in learned_lexicon[word]:
                learned_prons.append(("R", "N", stats_summed[(word, pron)], pron))
        learned_prons_sorted = sorted(
            learned_prons, key=lambda item: item[2], reverse=True
        )
        for item in learned_prons_sorted:
            print(
                "{} | {} |  {:.2f} | {}".format(item[0], item[1], item[2], item[3]),
                file=args.ref_lexicon_edits_handle,
            )

    num_oovs_with_acoustic_evidence = len(
        set(learned_lexicon.keys()).difference(set(ref_lexicon.keys()))
    )
    num_oovs = len(set(counts.keys()).difference(set(ref_lexicon.keys())))
    num_ivs = len(learned_lexicon) - num_oovs_with_acoustic_evidence
    print(
        "Average num. prons per word in the learned lexicon is {}".format(
            float(num_prons_tot) / float(len(learned_lexicon))
        ),
        file=sys.stderr,
    )
    ...
    print(
        "We have acoustic evidence for {} out of {} in-vocab (w.r.t the reference lexicon) words from the acoustic training data.".format(
            num_ivs, len(ref_lexicon)
        ),
        file=sys.stderr,
    )
    print(
        "  Among those frequent words whose counts in the training text > ",
        threshold,
        ":",
        file=sys.stderr,
    )
    ...
    print(
        "We have acoustic evidence for {} out of {} OOV (w.r.t the reference lexicon) words from the acoustic training data.".format(
            num_oovs_with_acoustic_evidence, num_oovs
        ),
        file=sys.stderr,
    )
    print(
        "  Among those words whose counts in the training text > {}:".format(threshold),
        file=sys.stderr,
    )
    print(
