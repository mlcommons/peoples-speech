...
def get_args():
    ...
    parser.add_argument(
        "--random-seed", type=int, dest="random_seed", default=123, help="Random seed."
    )
    parser.add_argument(
        "--modify-spk-id",
        type=str,
...
def copy_file_if_exists(
    input_file, output_file, utt_modifier_type, utt_modifier, fields=[0]
):
    if os.path.isfile(input_file):
        clean_dict = parse_file_to_dict(
            input_file, value_processor=lambda x: " ".join(x)
        )
        new_dict = {}
...
def main():
    ...
    if args.bg_noise_dir:
        ...
        bg_noise_reco2dur = parse_file_to_dict(
            args.bg_noise_dir + "/reco2dur", value_processor=lambda x: float(x[0])
        )
    ...
    if args.fg_noise_dir:
        ...
        fg_noise_reco2dur = parse_file_to_dict(
            args.fg_noise_dir + "/reco2dur", value_processor=lambda x: float(x[0])
        )
    ...
    copy_file_if_exists(
        input_dir + "/utt2num_frames",
        output_dir + "/utt2num_frames",
        args.utt_modifier_type,
        args.utt_modifier,
    )
    copy_file_if_exists(
    ...
    if args.modify_spk_id:
        copy_file_if_exists(
            input_dir + "/spk2gender",
            output_dir + "/spk2gender",
            args.utt_modifier_type,
            args.utt_modifier,
        )
    else:
        copy_file_if_exists(
            input_dir + "/spk2gender", output_dir + "/spk2gender", None, ""
        )
    ...
    if os.path.isfile(input_dir + "/utt2uniq"):
        ...
        create_augmented_utt2uniq(
            input_dir, output_dir, args.utt_modifier_type, args.utt_modifier
        )
