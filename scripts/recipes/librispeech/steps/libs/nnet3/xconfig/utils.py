...
def get_prev_names(all_layers, current_layer):
    ...
    for name in prev_names:
        if name in prev_names_set:
            raise RuntimeError(
                "{0}: Layer name {1} is used more than once.".format(sys.argv[0], name)
            )
...
def get_dim_from_layer_name(all_layers, current_layer, full_layer_name):
    ...
    for layer in all_layers:
        ...
        if layer.get_name() == layer_name:
            if (
                not auxiliary_output in layer.auxiliary_outputs()
                and auxiliary_output is not None
            ):
                raise RuntimeError(
                    "Layer '{0}' has no such auxiliary output:"
                    "'{1}' ({0}.{1})".format(layer_name, auxiliary_output)
                )
            return layer.output_dim(auxiliary_output)
    # No such layer was found.
    if layer_name in [layer.get_name() for layer in all_layers]:
        raise RuntimeError(
            "Layer '{0}' was requested before it appeared in "
...
def convert_value_to_type(key, dest_type, string_value):
    if dest_type == type(bool()):
        if string_value == "True" or string_value == "true":
            ...
            raise RuntimeError(
                "Invalid configuration value {0}={1} (expected bool)".format(
                    key, string_value
                )
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Invalid configuration value {0}={1} (expected int)".format(
                    key, string_value
                )
            )
...
class Descriptor(object):
    def __init__(self, descriptor_string=None, prev_names=None):
        ...
        if descriptor_string != None:
            try:
                ...
                raise RuntimeError(
                    "Error parsing Descriptor '{0}', specific error was: {1}".format(
    ...
    def config_string(self, layer_to_string):
        if self.operator is None:
            ...
            return (
                ...
                + ", ".join(
                    [
                        item.config_string(layer_to_string)
    ...
    def dim(self, layer_to_dim):
        if self.operator is None:
            ...
            for desc in self.items[1:]:
                ...
                if next_dim != dim:
                    raise RuntimeError(
                        "In descriptor {0}, different fields have different "
...
def expect_token(expected_item, seen_item, what_parsing):
    if seen_item != expected_item:
        raise RuntimeError(
            "parsing {0}, expected '{1}' but got '{2}'".format(
...
def is_valid_line_name(name):
    return (
        isinstance(name, str) and re.match(r"^[a-zA-Z_][-a-zA-Z_0-9.]*", name) != None
    )
    ...
    if first_token in [
        ...
        if first_token == "Offset":
            ...
            try:
                ...
                raise RuntimeError(
                    "Parsing Offset(), expected integer, got " + tokens[pos]
                )
            if tokens[pos] == ")":
                return (d, pos + 1)
            elif tokens[pos] != ",":
                raise RuntimeError(
                    "Parsing Offset(), expected ')' or ',', got " + tokens[pos]
                )
            pos += 1
            try:
            ...
            while True:
                if tokens[pos] == ")":
                    ...
                    if first_token == "Failover" and len(d.items) != 2:
                        raise RuntimeError(
                            "Parsing Failover(), expected 2 items but got {0}".format(
                                len(d.items)
                            )
                        )
                    if first_token == "IfDefined" and len(d.items) != 1:
                        raise RuntimeError(
                            "Parsing IfDefined(), expected 1 item but got {0}".format(
                                len(d.items)
                            )
                        )
                    pos += 1
                    ...
                    raise RuntimeError(
                        "Parsing Append(), expected ')' or ',', got " + tokens[pos]
                    )
            ...
            if tokens[pos] in ["x", "t"]:
                ...
                raise RuntimeError(
                    "Parsing ReplaceIndex(), expected 'x' or 't', got " + tokens[pos]
                )
            expect_token(",", tokens[pos], "ReplaceIndex()")
            pos += 1
            try:
        ...
        try:
            ...
            raise RuntimeError(
                "Parsing {0}, expected float, got {1}".format(first_token, tokens[pos])
            )
        # Consume the comma.
        expect_token(",", tokens[pos], first_token + "()")
        pos += 1
        if first_token == "Scale":
            ...
            try:
                ...
                raise RuntimeError(
                    "Parsing Const() expression, expected int, got {0}".format(
                        tokens[pos]
                    )
                )
        expect_token(")", tokens[pos], first_token)
        ...
        if tokens[pos] == "@":
            ...
            try:
                ...
                raise RuntimeError(
                    "Parse error parsing {0}@{1}".format(first_token, tokens[pos])
                )
            if offset_t != 0:
                inner_d = d
                d = Descriptor()
        ...
        try:
            ...
            raise RuntimeError(
                "Parsing descriptor, expected descriptor but got " + first_token
            )
        assert isinstance(prev_names, list)
        if len(prev_names) < 1:
            raise RuntimeError(
                "Parsing descriptor, could not interpret '{0}' because "
...
def replace_bracket_expressions_in_descriptor(descriptor_string, prev_names=None):
    ...
    while i < len(fields):
        ...
        if f == "]":
            ...
            if i + 2 >= len(fields):
                raise RuntimeError(
                    "Error tokenizing string '{0}': '[' found too close "
            ...
            try:
                ...
                raise RuntimeError(
                    "Error tokenizing string '{0}': expression [{1}] has an "
                    "invalid or out of range offset.".format(
                        descriptor_string, fields[i]
                    )
                )
...
#  [ 'Append', '(', '-1', ',', '0', ',', '1', ',', 'c', '@', '0', ')' ]
def tokenize_descriptor(descriptor_string, prev_names=None):
    ...
    fields = re.split(
        r"(\(|\)|@|,|\s)\s*",
        replace_bracket_expressions_in_descriptor(descriptor_string, prev_names),
    )
    ans = []
    for f in fields:
...
def parse_config_line(orig_config_line):
    ...
    if x is not None:
        ...
        if bad_char == "'":
            raise RuntimeError(
                "Xconfig line has disallowed character ' (use "
                "double quotes for strings containing = signs)"
            )
        else:
            raise RuntimeError(
                "Xconfig line has disallowed character: {0}".format(bad_char)
            )
    ...
    if re.match("^[a-z][-a-z0-9]+$", first_token) is None:
        raise RuntimeError(
            "Error parsing config line (first field doesn't look right)."
        )
    ...
    for i in range(num_strings):
        ...
        new_rest_of_line = (
            line_before_start + " " + inside_quotes + " " + line_after_end
        )
    ...
    for i in range(num_variables):
        ...
        if re.match(r"[a-zA-Z_]", var_name) is None:
            raise RuntimeError(
                "Expected variable name '{0}' to start with alphabetic character or _, "
                "in config line {1}".format(var_name, orig_config_line)
            )
        if var_name in ans_dict:
            raise RuntimeError(
                "Config line has multiply defined variable {0}: {1}".format(
...
def test_library():
    ...
    assert tokenize_descriptor("[-2].special@2", ["foo", "bar"])[:-1] == [
        "foo.special",
        "@",
        "2",
    ]
    ...
    assert (
        Descriptor("Sum(Offset(foo,1),Offset(foo,0))").str()
    ...
    for x, y in [
        ...
        ("[-2]@3", "Offset(last_but_one_layer, 3)"),
    ]:
