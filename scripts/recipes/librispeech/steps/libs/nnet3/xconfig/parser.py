...
def get_model_component_info(model_filename):
    ...
    try:
        ...
        sys.exit(
            "{0}: error reading model file '{1}'".format(
    ...
    out = common_lib.get_command_stdout(
        """nnet3-info {0} | grep '\-node' """ """ """.format(model_filename)
    )
    ...
    for line in out.split("\n"):
        ...
        if layer_name is not None and layer_name not in layer_names:
            ...
            all_layers.append(
                xlayers.XconfigExistingLayer("existing", key_to_value, all_layers)
            )
    if len(all_layers) == 0:
        raise RuntimeError(
            "{0}: model filename '{1}' is empty.".format(sys.argv[0], model_filename)
        )
    f.close()
    return all_layers
...
def read_xconfig_file(xconfig_filename, existing_layers=None):
    ...
    try:
        ...
        sys.exit(
            "{0}: error reading xconfig file '{1}'; error was {2}".format(
    ...
    if len(all_layers) == 0:
        raise RuntimeError(
            "{0}: xconfig file '{1}' is empty".format(sys.argv[0], xconfig_filename)
        )
    f.close()
    return all_layers
