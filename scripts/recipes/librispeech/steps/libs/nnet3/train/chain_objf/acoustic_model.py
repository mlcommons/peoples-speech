...
def create_phone_lm(dir, tree_dir, run_opts, lm_opts=None):
    ...
    try:
        ...
        raise Exception(
            """There was an error getting the number of alignment
                        jobs from {0}/num_jobs""".format(
                tree_dir
            )
        )

    alignments = " ".join(
        ["{0}/ali.{1}.gz".format(tree_dir, job) for job in range(1, num_ali_jobs + 1)]
    )
...
def create_denominator_fst(dir, tree_dir, run_opts):
    common_lib.execute_command(
        """copy-transition-model {tree_dir}/final.mdl \
                {dir}/0.trans_mdl""".format(
            dir=dir, tree_dir=tree_dir
        )
    )
    common_lib.execute_command(
...
def train_one_iteration(
    ...
    raw_model_string = (
        "nnet3-am-copy --raw=true --learning-rate={0} "
        "--scale={1} {2}/{3}.mdl - |".format(learning_rate, shrinkage_value, dir, iter)
    )
    ...
    train_new_models(
        ...
        backstitch_training_scale=(
            backstitch_training_scale * iter / 15
            if iter < 15
            else backstitch_training_scale
        ),
        backstitch_training_interval=backstitch_training_interval,
        use_multitask_egs=use_multitask_egs,
    )
...
def check_for_required_files(feat_dir, tree_dir, lat_dir=None):
    files = [
        "{0}/feats.scp".format(feat_dir),
        "{0}/ali.1.gz".format(tree_dir),
        "{0}/final.mdl".format(tree_dir),
        "{0}/tree".format(tree_dir),
    ]
    if lat_dir is not None:
        files += [
            "{0}/lat.1.gz".format(lat_dir),
...
def prepare_initial_acoustic_model(dir, run_opts, srand=-1, input_model=None):
    ...
    common_lib.execute_command(
        """{command} {dir}/log/init_mdl.log \
            ...
            dir=dir,
            raw_mdl=(
                input_model if input_model is not None else "{0}/0.raw".format(dir)
            ),
        )
    )
...
def combine_models(
    ...
    for iter in sorted(models_to_combine):
        ...
        if os.path.exists(model_file):
            ...
            print(
                "{0}: warning: model file {1} does not exist "
