...
def train_new_models(
    ...
    if max_deriv_time_relative is not None:
        deriv_time_opts.append(
            "--optimization.max-deriv-time-relative={0}".format(max_deriv_time_relative)
        )
    ...
    for job in range(1, num_jobs + 1):
        ...
        ) + (" --write-cache={0}/cache.{1}".format(dir, iter + 1) if job == 1 else "")

        if image_augmentation_opts:
            image_augmentation_cmd = "nnet3-egs-augment-image --srand={srand} {aug_opts} ark:- ark:- |".format(
                srand=k + srand, aug_opts=image_augmentation_opts
            )
        else:
        ...
        egs_rspecifier = """ark,bg:nnet3-copy-egs {frame_opts} {multitask_egs_opts} \
            ...
            frame_opts=("" if chunk_level_training else "--frame={0}".format(frame)),
            egs_dir=egs_dir,
            archive_index=archive_index,
...
def train_one_iteration(
    ...
    if os.path.exists("{0}/srand".format(dir)):
        ...
        if srand != saved_srand:
            logger.warning(
                "The random seed provided to this iteration "
                "(srand={0}) is different from the one saved last "
                "time (srand={1}). Using srand={0}.".format(srand, saved_srand)
            )
    else:
    ...
    raw_model_string = (
        "nnet3-copy --learning-rate={lr} --scale={s} "
        "{dir}/{iter}.{suf} - |".format(
            lr=learning_rate,
            s=shrinkage_value,
    ...
    if not os.path.isfile(new_model):
        raise Exception(
            "Could not find {0}, at the end of " "iteration {1}".format(new_model, iter)
        )
    elif os.stat(new_model).st_size == 0:
        raise Exception(
            "{0} has size 0. Something went wrong in "
...
def compute_preconditioning_matrix(
    ...
    common_lib.execute_command(
        """{command} {dir}/log/sum_transform_stats.log \
            ...
            command=run_opts.command, dir=dir, lda_stat_files=" ".join(lda_stat_files)
        )
    )
    ...
    for file in lda_stat_files:
        try:
            ...
            logger.error("There was error while trying to remove " "lda stat files.")
            raise
...
def compute_train_cv_probabilities(
    ...
    multitask_egs_opts = common_train_lib.get_multitask_egs_opts(
        egs_dir, egs_prefix="valid_diagnostic.", use_multitask_egs=use_multitask_egs
    )
    ...
    multitask_egs_opts = common_train_lib.get_multitask_egs_opts(
        egs_dir, egs_prefix="train_diagnostic.", use_multitask_egs=use_multitask_egs
    )
...
def compute_progress(dir, iter, egs_dir, run_opts, get_raw_nnet_from_am=True):
    ...
    if iter % 10 == 0 and iter > 0:
        ...
        """.format(
                command=run_opts.command, dir=dir, iter=iter, model=model
            )
        )
...
def combine_models(
    ...
    egs_rspecifier = "{0}:{1}/combine{2}".format(scp_or_ark, egs_dir, egs_suffix)
...
def get_realign_iters(realign_times, num_iters, num_jobs_initial, num_jobs_final):
    ...
    for realign_time in realign_times.split():
        ...
        if num_jobs_initial == num_jobs_final:
            ...
            realign_iter = math.sqrt(
                (1 - realign_time) * math.pow(num_jobs_initial, 2)
...
def align(dir, data, lang, run_opts, iter=None, online_ivector_dir=None):
    ...
    logger.info(
        "Aligning the data{gpu}with {num_jobs} jobs.".format(
    ...
    common_lib.execute_command(
        """steps/nnet3/align.sh --nj {num_jobs_align} \
            ...
            online_ivector_dir=(
                online_ivector_dir if online_ivector_dir is not None else ""
            ),
...
def realign(
    ...
    avg_post_vec_file = compute_average_posterior(
        dir=dir,
        iter=iter,
        egs_dir=prev_egs_dir,
        num_archives=num_archives,
        prior_subset_size=prior_subset_size,
        run_opts=run_opts,
    )
    ...
    alidir = align(dir, feat_dir, lang, run_opts, iter, online_ivector_dir)
    common_lib.execute_command(
...
def adjust_am_priors(dir, input_model, avg_posterior_vector, output_model, run_opts):
    common_lib.execute_command(
...
def compute_average_posterior(
    ...
    common_lib.execute_command(
        ...
        """.format(
            command=run_opts.command, dir=dir, iter=iter, output_file=avg_post_vec_file
        )
    )
