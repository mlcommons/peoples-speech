...
def get_multitask_egs_opts(
    egs_dir, egs_prefix="", archive_index=-1, use_multitask_egs=False
):
    ...
    if use_multitask_egs:
        output_file_name = "{egs_dir}/{egs_prefix}output{egs_suffix}.ark" "".format(
            egs_dir=egs_dir, egs_prefix=egs_prefix, egs_suffix=egs_suffix
        )
        output_rename_opt = ""
        ...
        weight_file_name = "{egs_dir}/{egs_prefix}weight{egs_suffix}.ark" "".format(
            egs_dir=egs_dir, egs_prefix=egs_prefix, egs_suffix=egs_suffix
        )
        weight_opt = ""
        ...
        multitask_egs_opts = "{output_rename_opt} {weight_opt}".format(
            output_rename_opt=output_rename_opt, weight_opt=weight_opt
        )
...
def get_successful_models(num_models, log_file_pattern, difference_threshold=1.0):
    ...
    if len(accepted_models) != num_models:
        logger.warn(
            "Only {0}/{1} of the models have been accepted "
            "for averaging, based on log files {2}.".format(
                len(accepted_models), num_models, log_file_pattern
            )
        )
...
def get_average_nnet_model(dir, iter, nnets_list, run_opts, get_raw_nnet_from_am=True):
    ...
    if get_raw_nnet_from_am:
        out_model = """- \| nnet3-am-copy --set-raw-nnet=-  \
            ...
            dir=dir, iter=iter, next_iter=next_iter
        )
    else:
        out_model = "{dir}/{next_iter}.raw".format(dir=dir, next_iter=next_iter)
...
def copy_egs_properties_to_exp_dir(egs_dir, dir):
    try:
        ...
        logger.error(
            "Error while trying to copy egs " "property files to {dir}".format(dir=dir)
        )
        raise
...
def get_input_model_info(input_model):
    ...
    try:
        out = common_lib.get_command_stdout(
            """nnet3-info {0} | """ """head -4 """.format(input_model)
        )
        # out looks like this
        # left-context: 7
        # right-context: 0
...
def verify_egs_dir(
    ...
    try:
        ...
        try:
            egs_ivector_id = (
                open("{0}/info/final.ie.id".format(egs_dir)).readline().strip()
            )
            if egs_ivector_id == "":
                egs_ivector_id = None
        except:
        ...
        try:
            egs_ivector_dim = int(
                open("{0}/info/ivector_dim".format(egs_dir)).readline()
            )
        except:
            egs_ivector_dim = 0
        egs_left_context = int(open("{0}/info/left_context".format(egs_dir)).readline())
        egs_right_context = int(
            open("{0}/info/right_context".format(egs_dir)).readline()
        )
        try:
            egs_left_context_initial = int(
                open("{0}/info/left_context_initial".format(egs_dir)).readline()
            )
        except:  # older scripts didn't write this, treat it as -1 in that case.
            egs_left_context_initial = -1
        try:
            egs_right_context_final = int(
                open("{0}/info/right_context_final".format(egs_dir)).readline()
            )
        except:  # older scripts didn't write this, treat it as -1 in that case.
        ...
        if (feat_dim != 0 and feat_dim != egs_feat_dim) or (
            ivector_dim != egs_ivector_dim
        ):
            raise Exception(
                "There is mismatch between featdim/ivector_dim of "
        ...
        if ((egs_ivector_id is None) and (ivector_extractor_id is not None)) or (
            ...
            logger.warning(
                "The ivector ids are used inconsistently. It's your "
                "responsibility to make sure the ivector extractor "
                "has been used consistently"
            )
            logger.warning(
                "ivector id for egs: {0} in dir {1}".format(egs_ivector_id, egs_dir)
            )
            logger.warning("ivector id for extractor: {0}".format(ivector_extractor_id))
        elif (
            (egs_ivector_dim > 0)
            and (egs_ivector_id is None)
            and (ivector_extractor_id is None)
        ):
            logger.warning(
                "The ivector ids are not used. It's your "
                "responsibility to make sure the ivector extractor "
                "has been used consistently"
            )
        elif ivector_extractor_id != egs_ivector_id:
            raise Exception(
                "The egs were generated using a different ivector "
        ...
        if egs_left_context < left_context or egs_right_context < right_context:
            raise Exception(
                "The egs have insufficient (l,r) context ({0},{1}) "
                "versus expected ({2},{3})".format(
                    egs_left_context, egs_right_context, left_context, right_context
                )
            )
        ...
        if (
            ...
            or egs_right_context_final != right_context_final
        ):
            raise Exception(
                "The egs have incorrect initial/final (l,r) context "
        ...
        frames_per_eg_str = (
            open("{0}/info/frames_per_eg".format(egs_dir)).readline().rstrip()
        )
        if not validate_chunk_width(frames_per_eg_str):
            raise Exception(
                "Invalid frames_per_eg in directory {0}/info".format(egs_dir)
            )
        num_archives = int(open("{0}/info/num_archives".format(egs_dir)).readline())

        return [egs_left_context, egs_right_context, frames_per_eg_str, num_archives]
    except (IOError, ValueError):
        logger.error(
            "The egs dir {0} has missing or " "malformed files.".format(egs_dir)
        )
        raise
...
def compute_presoftmax_prior_scale(
    ...
    common_lib.execute_command(
        """{command} JOB=1:{num_jobs} {dir}/log/acc_pdf.JOB.log \
                ...
                {dir}/pdf_counts.JOB""".format(
            command=run_opts.command, num_jobs=num_jobs, dir=dir, alidir=alidir
        )
    )
...
def smooth_presoftmax_prior_scale_vector(
    pdf_counts, presoftmax_prior_scale_power=-0.25, smooth=0.01
):
    ...
    for i in range(len(pdf_counts)):
        scales.append(
            math.pow(
                pdf_counts[i] + smooth * average_count, presoftmax_prior_scale_power
            )
        )
    ...
    if os.path.exists(dir + "/configs/init.config"):
        common_lib.execute_command(
            """{command} {dir}/log/add_first_layer.log \
                ...
                command=run_opts.command, srand=srand, dir=dir
            )
        )
    else:
        common_lib.execute_command(
...
def get_model_combine_iters(
    num_iters, num_epochs, num_archives, max_models_combine, num_jobs_final
):
    ...
    num_iters_combine_initial = min(
        int(approx_iters_per_epoch_final / 2) + 1, int(num_iters / 2)
    )
    ...
    if num_iters_combine_initial > max_models_combine:
        ...
        models_to_combine = set(
            range(
                num_iters - num_iters_combine_initial + 1,
                num_iters + 1,
                subsample_model_factor,
            )
        )
...
class CommonParser(object):
    ...
    def __init__(self, include_chunk_context=True, default_chunk_left_context=0):
        # feat options
        ...
        if include_chunk_context:
            ...
            self.parser.add_argument(
                "--egs.chunk-right-context",
                type=int,
                dest="chunk_right_context",
                default=0,
                help="""Number of additional frames of input
            ...
            self.parser.add_argument(
                "--egs.chunk-left-context-initial",
                type=int,
                dest="chunk_left_context_initial",
                default=-1,
                help="""Number of additional frames of input
            ...
            self.parser.add_argument(
                "--egs.chunk-right-context-final",
                type=int,
                dest="chunk_right_context_final",
                default=-1,
                help="""Number of additional frames of input
        ...
        self.parser.add_argument(
            ...
            dest="egs_stage",
            default=0,
            help="Stage at which get_egs.sh should be " "restarted",
        )
        self.parser.add_argument(
            "--egs.opts",
            type=str,
        ...
        self.parser.add_argument(
            "--trainer.max-param-change",
            type=float,
            dest="max_param_change",
            default=2.0,
        ...
        self.parser.add_argument(
            "--trainer.presoftmax-prior-scale-power",
            type=float,
        ...
        self.parser.add_argument(
            "--trainer.optimization.proportional-shrink",
            type=float,
            dest="proportional_shrink",
            default=0.0,
        ...
        self.parser.add_argument(
            "--trainer.optimization.initial-effective-lrate",
            type=float,
        ...
        self.parser.add_argument(
            "--trainer.optimization.final-effective-lrate",
            type=float,
        ...
        self.parser.add_argument(
            "--trainer.optimization.num-jobs-initial",
            type=int,
            dest="num_jobs_initial",
            default=1,
            help="Number of neural net jobs to run in "
            "parallel at the start of training",
        )
        self.parser.add_argument(
            "--trainer.optimization.num-jobs-final",
            type=int,
            dest="num_jobs_final",
            default=8,
            help="Number of neural net jobs to run in "
        ...
        self.parser.add_argument(
            "--trainer.optimization.num-jobs-step",
            type=positive_int,
            metavar="N",
            dest="num_jobs_step",
            default=1,
        ...
        self.parser.add_argument(
            "--trainer.optimization.max-models-combine",
            "--trainer.max-models-combine",
            type=int,
            dest="max_models_combine",
            default=20,
        ...
        self.parser.add_argument(
            "--trainer.optimization.max-objective-evaluations",
            "--trainer.max-objective-evaluations",
            type=int,
            dest="max_objective_evaluations",
            default=30,
        ...
        self.parser.add_argument(
            "--trainer.optimization.do-final-combination",
            dest="do_final_combination",
            type=str,
        ...
        self.parser.add_argument(
            "--trainer.optimization.combine-sum-to-one-penalty",
            type=float,
            dest="combine_sum_to_one_penalty",
            default=0.0,
            help="""This option is deprecated and does nothing.""",
        )
        self.parser.add_argument(
            "--trainer.optimization.momentum",
            type=float,
            dest="momentum",
            default=0.0,
        ...
        self.parser.add_argument(
            "--trainer.optimization.backstitch-training-scale",
            type=float,
            dest="backstitch_training_scale",
            default=0.0,
            help="""scale of parameters changes
                                 used in backstitch training step.""",
        )
        self.parser.add_argument(
            "--trainer.optimization.backstitch-training-interval",
            type=int,
            dest="backstitch_training_interval",
            default=1,
            help="""the interval of minibatches
                                 that backstitch training is applied on.""",
        )
        self.parser.add_argument(
            "--trainer.compute-per-dim-accuracy",
            dest="compute_per_dim_accuracy",
            type=str,
            choices=["true", "false"],
            default=False,
            action=common_lib.StrToBoolAction,
            help="Compute train and validation " "accuracy per-dim",
        )
        ...
        self.parser.add_argument(
            ...
            help="Use GPU for training. " "Note 'true' and 'false' are deprecated.",
            default="yes",
        )
        self.parser.add_argument(
            "--cleanup",
        ...
        self.parser.add_argument(
            "--cleanup.preserve-model-interval",
...
class SelfTest(unittest.TestCase):
    def test_halve_minibatch_size_str(self):
        ...
        self.assertEqual(
            "128=32/256=20,40:50", halve_minibatch_size_str("128=64/256=40,80:100")
        )
    ...
    def test_get_current_num_jobs(self):
        niters = 12
        self.assertEqual(
            [2, 3, 3, 4, 4, 5, 6, 6, 7, 7, 8, 8],
            [get_current_num_jobs(i, niters, 2, 1, 9) for i in range(niters)],
        )
        self.assertEqual(
            [2, 3, 3, 3, 3, 6, 6, 6, 6, 6, 9, 9],
            [get_current_num_jobs(i, niters, 2, 3, 9) for i in range(niters)],
        )
