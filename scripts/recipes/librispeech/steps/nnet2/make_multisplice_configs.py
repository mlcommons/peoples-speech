...
def parse_splice_string(splice_string):
    ...
    try:
        for cur_splice_indexes in layerwise_splice_indexes:
            ...
            if layer_index == 0:
                ...
                try:
                    assert frame_indexes == list(
                        range(first_left_context, first_right_context + 1)
                    )
                except AssertionError:
                    raise Exception(
                        "Currently the first splice component just accepts contiguous context."
                    )
                try:
          ...
          left context provided is %d and right context provided is %d."""
            ...
            nnet_frame_indexes = get_convolution_index_set(
                frame_indexes, nnet_frame_indexes
            )
            cur_context = ":".join([str(x) for x in frame_indexes])
            contexts[layer_index] = cur_context
    except ValueError:
        raise Exception(
            "Unknown format in splice_indexes variable: {0}".format(
                params.splice_indexes
            )
        )
    ...
    return [
        ...
        " nnet_left_context={0};\n nnet_right_context={1}\n first_left_context={2};\n first_right_context={3}\n".format(
            abs(max_left_context),
            abs(max_right_context),
            abs(first_left_context),
            abs(first_right_context),
        ),
    ]
...
def create_config_files(output_dir, params):
    ...
    try:
        nnet_config = [
            "SpliceComponent input-dim={0} context={1} const-component-dim={2}".format(
                params.total_input_dim, contexts[0], params.ivector_dim
            ),
            "FixedAffineComponent matrix={0}".format(params.lda_mat),
            "AffineComponentPreconditionedOnline input-dim={0} output-dim={1} {2} learning-rate={3} param-stddev={4} bias-stddev={5}".format(
                params.lda_dim,
                pnorm_input_dim,
                params.online_preconditioning_opts,
                params.initial_learning_rate,
                stddev,
                params.bias_stddev,
            ),
            (
                "PnormComponent input-dim={0} output-dim={1} p={2}".format(
                    pnorm_input_dim, pnorm_output_dim, pnorm_p
                )
                if pnorm_input_dim != pnorm_output_dim
                else "RectifiedLinearComponent dim={0}".format(pnorm_input_dim)
            ),
            "NormalizeComponent dim={0}".format(pnorm_output_dim),
            "AffineComponentPreconditionedOnline input-dim={0} output-dim={1} {2} learning-rate={3} param-stddev=0 bias-stddev=0".format(
                pnorm_output_dim,
                params.num_targets,
                params.online_preconditioning_opts,
                params.initial_learning_rate,
        ...
        raise Exception(
            "A splice layer is expected to be the first layer. Provide a context for the first layer."
        )

    for i in range(
        1, params.num_hidden_layers
    ):  # just run till num_hidden_layers-1 since we do not add splice before the final affine transform
        lines = []
        context_len = 1
        if i in contexts:
            # Adding the splice component as a context is provided
            lines.append(
                "SpliceComponent input-dim=%d context=%s "
                % (pnorm_output_dim, contexts[i])
            )
        ...
        lines.append(
            "AffineComponentPreconditionedOnline input-dim=%d output-dim=%d %s learning-rate=%f param-stddev=%f bias-stddev=%f"
            % (
                pnorm_output_dim * context_len,
                pnorm_input_dim,
                params.online_preconditioning_opts,
                params.initial_learning_rate,
                stddev,
        ...
        if pnorm_input_dim != pnorm_output_dim:
            lines.append(
                "PnormComponent input-dim=%d output-dim=%d p=%d"
                % (pnorm_input_dim, pnorm_output_dim, pnorm_p)
            )
        else:
            lines.append("RectifiedLinearComponent dim=%d" % (pnorm_input_dim))
            warnings.warn(
                "Using the RectifiedLinearComponent, in place of the PnormComponent as pnorm_input_dim == pnorm_output_dim"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--splice-indexes",
        type=str,
        help="string specifying the indexes for the splice layers throughout the network",
    )
    parser.add_argument(
        "--total-input-dim", type=int, help="dimension of the input to the network"
    )
    parser.add_argument(
        "--ivector-dim",
        type=int,
        help="dimension of the ivector portion of the neural network input",
    )
    parser.add_argument(
        "--lda-mat", type=str, help="lda-matrix used after the first splice component"
    )
    parser.add_argument("--lda-dim", type=str, help="dimension of the lda output")
    parser.add_argument(
        "--pnorm-input-dim", type=int, help="dimension of input to pnorm layer"
    )
    parser.add_argument(
        "--pnorm-output-dim", type=int, help="dimension of output of pnorm layer"
    )
    parser.add_argument(
        "--online-preconditioning-opts",
        type=str,
        help="extra options for the AffineComponentPreconditionedOnline component",
    )
    parser.add_argument("--initial-learning-rate", type=float, help="")
    parser.add_argument(
        "--num-targets", type=int, help="#targets for the neural network "
    )
    parser.add_argument(
        "--num-hidden-layers", type=int, help="#hidden layers in the neural network "
    )
    parser.add_argument(
        "--bias-stddev",
        type=float,
        help="standard deviation of r.v. used for bias component initialization",
    )
    parser.add_argument("mode", type=str, help="contexts|configs")
    parser.add_argument(
        "output_dir", type=str, help="output directory to store the files"
    )
