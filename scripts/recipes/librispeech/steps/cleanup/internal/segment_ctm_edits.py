...
class Segment(object):
    ...
    def DebugInfo(self):
        return (
            "start=%d,end=%d,unk-padding=%.2f,%.2f,keep-proportion=%.2f,%.2f,"
            % (
                self.start_index,
                self.end_index,
                self.start_unk_padding,
    ...
    def PossiblyTruncateStartForJunkProportion(self):
        ...
        for i in range(self.start_index + 1, self.end_index - 1):
            ...
            if (
                this_edit_type == "sil"
                or (this_edit_type == "cor" and this_ref_word in non_scored_words)
            ) and (float(this_split_line[3]) > args.min_split_point_duration):
        ...
        if (
            float(begin_junk_duration) / candidate_removed_piece_duration
    ...
    def PossiblyTruncateEndForJunkProportion(self):
        ...
        if (
            float(end_junk_duration) / candidate_removed_piece_duration
            < args.max_junk_proportion
        ):
            return  # Nothing to do as the candidate piece to remove has too
            # little junk.
    ...
    def ContainsAtLeastOneScoredNonOovWord(self):
        ...
        for i in range(self.start_index, self.end_index):
            ...
            if (
                this_edit == "cor"
                and not this_ref_word in non_scored_words
...
def PrintSegmentStats():
    global segment_total_length, num_segments, num_utterances, num_utterances_without_segments, total_length_of_utterances
...
def WriteSegmentsForUtterance(
    text_output_handle, segments_output_handle, old_utterance_name, segments
):
...
def ProcessData():
    try:
        ...
        sys.exit(
            "segment_ctm_edits.py: error opening ctm-edits input "
    ...
    try:
        ...
        sys.exit(
            "segment_ctm_edits.py: error opening text output "
    ...
    try:
        ...
        sys.exit(
            "segment_ctm_edits.py: error opening segments output "
            "file {0}".format(args.text_out)
        )
    if args.ctm_edits_out != None:
        try:
            ...
            sys.exit(
                "segment_ctm_edits.py: error opening ctm-edits output "
    ...
    try:
        ...
        sys.exit(
            "segment_ctm_edits.py: error closing one or more outputs "
...
def ReadNonScoredWords(non_scored_words_file):
    ...
    try:
        ...
        sys.exit(
            "segment_ctm_edits.py: error opening file: "
...
if args.oov_symbol_file != None:
    try:
        ...
        sys.exit(
            "segment_ctm_edits.py: error reading file --oov-symbol-file="
            + args.oov_symbol_file
            + ", error is: "
            + str(e)
        )
