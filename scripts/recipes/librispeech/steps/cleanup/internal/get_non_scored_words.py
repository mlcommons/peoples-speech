...
def read_lang(lang_dir):
    ...
    if not os.path.isdir(lang_dir):
        logger.error("expected lang/ directory %s to " "exist.", lang_dir)
        raise RuntimeError
    ...
    try:
        ...
        logger.error("problem reading file " "%s/phones/silence.int", lang_dir)
        raise
    ...
    try:
        ...
        for line in open(lang_dir + "/phones/align_lexicon.int").readlines():
            ...
            if (
                ...
                and a[0] == a[1]
                and int(a[0]) > 0
                and int(a[2]) in silence_phones
            ):
                silence_word_ints.add(int(a[0]))
    except Exception:
        logger.error("problem reading file %s/phones/align_lexicon.int", lang_dir)
        raise
    ...
    if not len(non_scored_words) == len(silence_word_ints):
        raise RuntimeError(
            "error getting silence words, len({0}) != len({1})"
