...
def get_args():
    ...
    parser.add_argument(
        "segments", type=argparse.FileType("r"), help="use segments to resolve overlaps"
    )
    parser.add_argument(
        "ctm_edits_in", type=argparse.FileType("r"), help="input_ctm_file"
    )
    parser.add_argument(
        "ctm_edits_out", type=argparse.FileType("w"), help="output_ctm_file"
    )
    parser.add_argument(
        "--verbose", type=int, default=0, help="Higher value for more verbose logging."
    )
...
def read_segments(segments_file):
    ...
    logger.info("Read %d lines from segments file %s", num_lines, segments_file.name)
    segments_file.close()
...
def choose_best_ctm_lines(first_lines, second_lines, window_length, overlap_length):
    """Returns ctm lines that have lower WER. If the WER is the lines with
    the higher number of words is returned.
    """
    i, best_lines = min((0, first_lines), (1, second_lines), key=lambda x: wer(x[1]))
    return i
        ...
        try:
            ...
            try:
                cur_utt_end_index = next(
                    (
                        i
                        for i, line in enumerate(ctm_edits_for_cur_utt)
            ...
            try:
                next_utt_start_index = next(
                    (
                        i
                        for i, line in enumerate(ctm_edits_for_next_utt)
            ...
            choose_index = choose_best_ctm_lines(
                cur_utt_end_lines, next_utt_start_lines, window_length, overlap
            )
            ...
            logger.error(
                "Could not resolve overlaps between CTM edits for " "%s and %s",
                cur_utt,
                next_utt,
            )
...
def ctm_edit_line_to_string(line):
    ...
    return "{0} {1} {2} {3} {4} {5} {6}".format(
        line[0], line[1], line[2], line[3], line[4], line[5], " ".join(line[6:])
    )
