...
def GetStatsFromCtmProns(
    silphones, optional_silence, non_scored_words, ctm_prons_file_handle
):
    info = []
    for line in ctm_prons_file_handle.readlines():
        ...
        if word == "<eps>" or (
            word in non_scored_words and word != "<unk>" and word != "<UNK>"
        ):
            nonsil_left = []
            nonsil_right = []
            for phone in phones:
            ...
            if (
                len(info) > 0
                and utt == info[-1][0]
                and isinstance(info[-1][2], str)
                and (phones == [] or phones[0] not in silphones)
            ):
...
def Main():
    ...
    stats = GetStatsFromCtmProns(
        silphones, optional_silence.pop(), non_scored_words, args.ctm_prons_file_handle
    )
