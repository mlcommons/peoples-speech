...
def get_args():
    ...
    parser.add_argument(
        "--splitting.min-non-scored-length",
    ...
    parser.add_argument(
        "--splitting.hard-max-segment-length",
    ...
    parser.add_argument(
        "--merging-score.incorrect-words-factor",
    ...
    parser.add_argument(
        "--merging-score.tainted-words-factor",
    ...
    parser.add_argument(
        "--merging.max-intersegment-incorrect-words-length",
...
def compute_segment_cores(split_lines_of_utt):
    ...
    for i in range(1, num_lines):
        if line_is_in_segment_core[i - 1] and not line_is_in_segment_core[i]:
            ...
            if not is_tainted(split_lines_of_utt[i]) and (
...
class SegmentStats(object):
    ...
    def bad_proportion(self):
        ...
        proportion = (
            float(
                self.silence_length
                + self.tainted_nonsilence_length
                + self.incorrect_words_length
            )
            / self.total_length
        )
    ...
    def combine(self, other, scale=1):
        ...
        self.tainted_nonsilence_length += scale * other.tainted_nonsilence_length
    ...
    def assert_equal(self, other):
        try:
            ...
            assert (
                abs(self.incorrect_words_length - other.incorrect_words_length) < 0.01
            )
            assert (
                abs(self.tainted_nonsilence_length - other.tainted_nonsilence_length)
                < 0.01
            )
    ...
    def __str__(self):
        return (
            "num-incorrect-words={num_incorrect:d},"
...
class Segment(object):
    ...
    def copy(self, copy_stats=True):
        segment = Segment(
            ...
            segment_stats=(None if not copy_stats else copy.deepcopy(self.stats)),
        )
    ...
    def compute_stats(self):
        ...
        for i in range(self.start_index, self.end_index):
            ...
            try:
                ...
                if (
                    is_tainted(self.split_lines_of_utt[i])
                    and self.split_lines_of_utt[i][7] not in "sil"
                    and (self.split_lines_of_utt[i][6] not in non_scored_words())
                ):
                ...
                _global_logger.error(
                    "Something went wrong when computing stats at " "ctm line %s",
        ...
        try:
            assert (
                self.stats.tainted_nonsilence_length
                + self.stats.silence_length
                + self.stats.incorrect_words_length
                - 0.001
                <= self.stats.total_length
            )
        except AssertionError:
            _global_logger.error("Something wrong with the stats for segment %s", self)
            raise
            ...
            if adjacent_index >= 0 and adjacent_index < len(split_lines_of_utt):
                ...
                if adjacent_line_is_tainted:
                    ...
                    if boundary_edit_type == "cor" and (
                        boundary_ref_word not in non_scored_words()
                    ):
    ...
    def possibly_split_segment(
        self, max_internal_silence_length, max_internal_non_scored_length
    ):
        """
        ...
        for index_to_split_at in range(cur_start_index + 1, self.end_index - 1):
            ...
            if (
                this_edit_type == "sil" and this_duration > max_internal_silence_length
            ) or (
                this_ref_word in non_scored_words()
                and (this_duration > max_internal_non_scored_length)
            ):
    ...
    def possibly_split_long_segment(
        self,
        max_segment_length,
        ...
        for index_to_split_at in range(cur_start_index + 1, self.end_index - 1):
            ...
            if this_edit_type == "sil" and this_duration > min_silence_length_to_split:
                split_indexes.append(
                    (index_to_split_at, this_duration, this_is_tainted)
                )

            if this_ref_word in non_scored_words() and (
                this_duration > min_non_scored_length_to_split
            ):
                split_indexes.append(
                    (index_to_split_at, this_duration, this_is_tainted)
                )
            ...
            try:
                while True:
                    index_to_split_at = next(
                        (
                            i
                            for i in range(cur_start_index, segment.end_index)
                            if (
                                float(self.split_lines_of_utt[i][2])
                    ...
                    new_segment = Segment(
                        self.split_lines_of_utt, cur_start_index, index_to_split_at
                    )
                    new_segments.append(new_segment)

                    cur_start_index = index_to_split_at
                    cur_start = float(self.split_lines_of_utt[cur_start_index][2])
                    index_to_split_at = None

                    if segment.end_time() - cur_start < hard_max_segment_length:
                        raise StopIteration
            except StopIteration:
                ...
                new_segment = Segment(
                    self.split_lines_of_utt, cur_start_index, segment.end_index
                )
                new_segments.append(new_segment)
                break
        segments = new_segments
    ...
    def possibly_truncate_boundaries(
        ...
        for b in [True, False]:
            ...
            this_ref_word = this_split_line[6]
            if this_edit == "sil" and this_duration > max_edge_silence_length:
    ...
    def relax_boundary_truncation(self, min_segment_length, min_new_segment_length):
        """
    ...
    def merge_with_segment(self, other, max_deleted_words):
        ...
        assert (
            not self.stats.compare(other.stats)
        ...
        try:
            ...
            _global_logger.error("first-index-of-overlap = %d", first_index_of_overlap)
            _global_logger.error("last-index-of-overlap = %d", last_index_of_overlap)
            _global_logger.error("line = %d = %s", i, self.split_lines_of_utt[i])
            raise
        _global_logger.debug("After merging %s", self)
    ...
    def contains_atleast_one_scored_non_oov_word(self):
        ...
        for i in range(self.start_index, self.end_index):
            ...
            if (
                this_edit == "cor"
                and this_ref_word not in non_scored_words()
...
class SegmentsMerger(object):
    ...
    def __init__(self, segments):
        ...
        if segments[-1].end_index < len(self.split_lines_of_utt):
            self.between_segments[-1] = Segment(
                self.split_lines_of_utt,
                segments[-1].end_index,
                len(self.split_lines_of_utt),
                compute_segment_stats=True,
            )

    def _get_merged_cluster(
        self,
        cluster1,
        cluster2,
        rejected_clusters=None,
        ...
        try:
            ...
            if rejected_clusters is not None and new_cluster_tup in rejected_clusters:
                return (None, new_cluster, True)

            if new_cluster_tup in self.merged_segments:
                return (self.merged_segments[new_cluster_tup], new_cluster, False)
            ...
            if cluster1[-1] == -1:
                ...
                # segment
                if (
                    self.between_segments[0] is None
                    or self.between_segments[0].stats.total_length == 0
                    or (
                        self.between_segments[0].stats.incorrect_words_length
                ...
                if cluster2[0] == len(self.segments):
                    ...
                    if (
                        self.between_segments[-1] is None
                        or (self.between_segments[-1].stats.total_length == 0)
                        or (
                            self.between_segments[-1].stats.incorrect_words_length
                            > max_intersegment_incorrect_words_length
                        )
                    ):
                ...
                if self.between_segments[cluster2[0]] is not None:
                    if (
                        self.between_segments[cluster2[0]].stats.incorrect_words_length
                        > max_intersegment_incorrect_words_length
                    ):
                        return (None, new_cluster, True)
            ...
            _global_logger.error(
                "Failed merging cluster1 %s and cluster2 %s", cluster1, cluster2
            )
            for i in cluster1 + cluster2:
                if i >= 0 and i < len(self.segments):
                    _global_logger.error("Segment %d = %s", i, self.segments[i])
            raise
    ...
    def merge_clusters(
        ...
        scoring_function,
        max_wer=10,
        max_bad_proportion=0.3,
        max_segment_length=10,
        max_intersegment_incorrect_words_length=1,
    ):
        for i, x in enumerate(self.segments):
            _global_logger.debug(
                "before agglomerative clustering, segment %d" " = %s", i, x
            )
        ...
        while len(clusters) > 1:
            try:
                ...
                for i in range(len(clusters) - 1):
                    merged_segment, new_cluster, reject = self._get_merged_cluster(
                        clusters[i],
                        clusters[i + 1],
                        rejected_clusters,
                        max_intersegment_incorrect_words_length=(
                    ...
                    heapq.heappush(
                        heap,
                        (
                            (-scoring_function(merged_segment), i),
                ...
                while True:
                    ...
                    if segment.stats.wer() > max_wer:
                        _global_logger.debug(
                            "Rejecting cluster with " "WER%% %.2f > %.2f",
                            segment.stats.wer(),
                            max_wer,
                        )
                    ...
                    if segment.stats.bad_proportion() > max_bad_proportion:
                        _global_logger.debug(
                            "Rejecting cluster with bad-proportion " "%.2f > %.2f",
                            segment.stats.bad_proportion(),
                            max_bad_proportion,
                        )
                        rejected_clusters.add(tuple(cluster))
                        continue
                    ...
                    if segment.stats.total_length > max_segment_length:
                        _global_logger.debug(
                            "Rejecting cluster with length " "%.2f > %.2f",
                            segment.stats.total_length,
                            max_segment_length,
                        )
                        rejected_clusters.add(tuple(cluster))
                        continue
                    ...
                    _global_logger.debug(
                        "Accepted cluster (%d, %s)", candidate_index, candidate_cluster
                    )
                    break
                ...
                if len(new_clusters) >= len(clusters):
                    raise RuntimeError(
                        "Old: {0}; New: {1}".format(clusters, new_clusters)
                    )
                clusters = new_clusters
            except Exception:
                _global_logger.error("Failed merging clusters %s", clusters)
                raise
...
def merge_segments(segments, args):
    ...
    for cluster_index, cluster in enumerate(clusters):
        _global_logger.debug("Merging cluster (%d, %s)", cluster_index, cluster)

        try:
            if cluster_index == 0 and len(cluster) == 1:
                assert cluster[0] == -1
                _global_logger.debug("Not adding region before the first segment")
                ...
                _global_logger.debug("Not adding remaining end region %s", cluster[0])
            ...
            _global_logger.error("Error with cluster (%d, %s)", cluster_index, cluster)
            raise

    segments = new_segments

    for i, x in enumerate(segments):
        _global_logger.debug("after agglomerative clustering: segment %d = %s", i, x)
    ...
    if segment_index == len(segments):
        _global_logger.debug(
            "No merged segments were below " "WER%% %.2f", args.max_wer
        )
        return []

    _global_logger.debug(
        "Merging overlapping segments starting from the "
...
def get_segments_for_utterance(split_lines_of_utt, args, utterance_stats):
    ...
    segments = [Segment(split_lines_of_utt, x[0], x[1]) for x in segment_ranges]

    utterance_stats.accumulate_segment_stats(segments, "stage  0 [segment cores]")
    ...
    if args.verbose > 4:
        ...
        print_debug_info_for_utterance(
            sys.stderr, copy.deepcopy(split_lines_of_utt), segments_copy, []
        )

    for segment in segments:
        segment.possibly_add_tainted_lines()
    utterance_stats.accumulate_segment_stats(segments, "stage  1 [add tainted lines]")
    ...
    utterance_stats.accumulate_segment_stats(segments, "stage  2 [merge segments]")
    ...
    for s in segments:
        new_segments += s.possibly_split_segment(
            args.max_internal_silence_length, args.max_internal_non_scored_length
        )
    segments = new_segments
    utterance_stats.accumulate_segment_stats(segments, "stage  3 [split segments]")

    for i, x in enumerate(segments):
        _global_logger.debug("stage 3: segment %d, %s", i, x.debug_info(False))
    ...
    segments = new_segments
    utterance_stats.accumulate_segment_stats(segments, "stage  4 [split long segments]")

    for i, x in enumerate(segments):
        _global_logger.debug("stage 4: segment %d, %s", i, x.debug_info(False))
    ...
    for s in segments:
        s.possibly_truncate_boundaries(
            args.max_edge_silence_length, args.max_edge_non_scored_length
        )
    utterance_stats.accumulate_segment_stats(segments, "stage  5 [truncate boundaries]")

    for i, x in enumerate(segments):
        _global_logger.debug("stage 5: segment %d = %s", i, x.debug_info(False))
    ...
    for i, x in enumerate(segments):
        _global_logger.debug("stage 6: segment %d = %s", i, x.debug_info(False))
    ...
    utterance_stats.accumulate_segment_stats(segments, "stage  7 [unk-padding]")

    for i, x in enumerate(segments):
        _global_logger.debug("stage 7: segment %d = %s", i, x.debug_info(False))
    ...
    segments = new_segments
    utterance_stats.accumulate_segment_stats(
        segments, "stage  8 [remove new segments under --min-new-segment-length"
    )

    for i, x in enumerate(segments):
        _global_logger.debug("stage 8: segment %d = %s", i, x.debug_info(False))
    ...
    for i, x in enumerate(segments):
        _global_logger.debug("stage 9: segment %d = %s", i, x.debug_info(False))

    if args.verbose > 4:
        print(
            "Stage 9 [remove segments under " "--min-segment-length]:", file=sys.stderr
        )
        segments_copy = [x.copy() for x in segments]
        print_debug_info_for_utterance(
            sys.stderr, copy.deepcopy(split_lines_of_utt), segments_copy, []
    ...
    for i, x in enumerate(segments):
        _global_logger.debug("stage 10: segment %d = %s", i, x.debug_info(False))

    if args.verbose > 4:
        print(
            "Stage 10 [remove segments without scored, non-OOV words " "",
            file=sys.stderr,
        )
        segments_copy = [x.copy() for x in segments]
        print_debug_info_for_utterance(
...
def write_segments_for_utterance(
    text_output_handle,
    ...
    for n, segment in enumerate(segments):
        ...
        print(
            new_utterance_name,
            segment.text(oov_symbol, eps_symbol),
...
def print_debug_info_for_utterance(
    ...
    for n, segment in enumerate(segments_for_utterance):
        start_string = "start-segment-{0}[{1}]".format(n + 1, segment.debug_info())
    ...
    for i, split_line in enumerate(split_lines_of_cur_utterance):
        ...
        while len(info_to_print) > 0 and info_to_print[0][0] <= end_time:
            ...
            split_line_copy.append(
                "{0}={1}".format(string, time_to_string(segment_start, frame_length))
            )
...
class WordStats(object):
    ...
    def accumulate_for_utterance(
        self, split_lines_of_utt, segments_for_utterance, eps_symbol="<eps>"
    ):
    ...
    def print(self, word_stats_out):
        ...
        try:
            ...
            _global_logger.error(
                "error closing file --word-stats-out=%s " "(full disk?)",
                word_stats_out.name,
            )
            raise
...
def process_data(args, oov_symbol, utterance_stats, word_stats):
    ...
    while True:
        try:
            ...
            _global_logger.error("Error with utterance %s", cur_utterance)
            raise
    ...
    try:
        ...
        if args.oov_symbol_file is not None:
            try:
                ...
                _global_logger.error(
                    "error reading file " "--oov-symbol-file=%s",
        ...
        try:
            ...
            _global_logger.error(
                "error closing one or more outputs " "(broken pipe or full disk?)"
            )
            raise
        ...
        if args.ctm_edits_out is not None:
            _global_logger.info(
                "detailed utterance-level debug information " "is in %s",
                args.ctm_edits_out.name,
            )
    except:
