...
parser.add_argument(
    "ctm_in", metavar="<ctm-in>", help="Filename of input hypothesis in ctm format"
)
parser.add_argument(
    "ctm_edits_out",
...
def OpenFiles():
    ...
    try:
        ...
        sys.exit(
            "get_ctm_edits.py: error opening ctm-edits file {0} for output".format(
                args.ctm_edits_out
            )
        )
    try:
        ...
        sys.exit(
            "get_ctm_edits.py: error opening edits file {0} for input".format(
                args.edits_in
            )
        )
    try:
        ...
        sys.exit(
            "get_ctm_edits.py: error opening ctm file {0} for input".format(args.ctm_in)
        )
    ...
    if args.symbol_table != None:
        if args.oov == -1:
            print(
                "get_ctm_edits.py: error: if you set the the --symbol-table option "
                "you must also set the --oov option",
                file=sys.stderr,
            )
        try:
            ...
            sys.exit(
                "get_ctm_edits.py: error opening symbol-table file {0} for "
                "input (or bad file), exception is: {1}".format(args.symbol_table)
            )
        f.close()
        if oov_word == None:
            sys.exit(
                "get_ctm_edits.py: OOV word not found: check the values of "
                "--symbol-table={0} and --oov={1}".format(args.symbol_table, args.oov)
            )
...
def PadArrays(edits_array, ctm_array):
    ...
    while edits_pos < edits_len or ctm_pos < ctm_len:
        if (
            edits_pos < edits_len
            and ctm_pos < ctm_len
            and edits_array[edits_pos][0] == ctm_array[ctm_pos][2]
            and edits_array[edits_pos][0] != "<eps>"
        ):
            ...
            raise Exception(
                "Could not align edits_array = {0} and ctm_array = {1}; "
...
def GetEditType(hyp_word, ref_word, duration):
    ...
    if hyp_word == ref_word and hyp_word != "<eps>":
        ...
        hyp_word == oov_word and len(symbol_table) != 0 and not ref_word in symbol_table
    ):
...
def OutputCtm(utterance_id, edits_array, ctm_array):
    ...
    for i in range(len(edits_array)):
        ...
        if not hyp_word == hyp_word2:
            print(
                "Error producing output CTM for edit = {0} and ctm = {1}".format(
                    edits_array[i], ctm_array[i]
                ),
                file=sys.stderr,
            )
            sys.exit(1)
...
def ProcessOneUtterance(utterance_id, edits_line, ctm_lines):
    try:
        ...
        if len(first_fields) != len(second_fields) or (
            len(fields_split) >= 3 and set(fields_split[2::3]) != {";"}
        ):
            sys.exit(
                "get_ctm_edits.py: could not make sense of edits line: " + edits_line
            )
        ...
        for line in ctm_lines:
            try:
                ...
                sys.exit(
                    "get_ctm_edits.py: error procesing ctm line {0} "
        ...
        sys.exit(
            "get_ctm_edits.py: error processing utterance {0}, error was: {1}".format(
                utterance_id, str(e)
            )
        )
...
def ProcessData():
    ...
    while True:
        ...
        if this_edits_line == "":
            if pending_ctm_line != "":
                sys.exit(
                    "get_ctm_edits.py: edits_in input {0} ended before "
        ...
        if len(a) == 0:
            sys.exit(
                "get_ctm_edits.py: edits_input {0} had an empty line".format(
                    args.edits_in
                )
            )
        utterance_id = a[0]
        ...
        while (
            len(pending_ctm_line.strip()) > 0
            and pending_ctm_line.split()[0] == utterance_id
        ):
            this_utterance_ctm_lines.append(pending_ctm_line)
            pending_ctm_line = ctm_in.readline()
        ProcessOneUtterance(utterance_id, this_edits_line, this_utterance_ctm_lines)
        num_utterances_processed += 1
    print(
        "get_ctm_edits.py: processed {0} utterances".format(num_utterances_processed),
        file=sys.stderr,
    )
