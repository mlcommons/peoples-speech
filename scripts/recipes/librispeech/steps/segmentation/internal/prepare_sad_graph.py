...
def get_args():
    ...
    parser.add_argument(
        "--frame-shift", type=float, default=0.03, help="""Frame shift in seconds"""
    )

    parser.add_argument(
        "--edge-silence-probability",
        type=float,
        default=0.5,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Transition probability for silence to speech " "or vice-versa",
    )

    parser.add_argument("output_graph", type=str, help="Output graph")
    args = parser.parse_args()

    args.min_states_silence = int(args.min_silence_duration / args.frame_shift + 0.5)
    args.min_states_speech = int(args.min_speech_duration / args.frame_shift + 0.5)
    args.max_states_speech = int(args.max_speech_duration / args.frame_shift + 0.5)
...
def print_states(args, file_handle):
    ...
    print(
        "0 1 silence silence {0}".format(-math.log(args.edge_silence_probability)),
        file=file_handle,
    )
    ...
    for state in range(
        silence_start_state, silence_start_state + args.min_states_silence - 1
    ):
        print(
            "{state} {next_state} silence silence {cost}".format(
    ...
    # Silence self-loop
    print(
        "{state} {state} silence silence {cost}".format(
    ...
    print(
        "0 {state} speech speech {cost}".format(
    ...
    print(
        "{sil_state} {speech_state} speech speech {cost}".format(
    ...
    # Speech min duration
    for state in range(
        speech_start_state, speech_start_state + args.min_states_speech - 1
    ):
        print(
            "{state} {next_state} speech speech {cost}".format(
                state=state, next_state=state + 1, cost=0.0
            ),
            file=file_handle,
        )
    ...
    for state in range(
        ...
        print(
            "{state} {sil_state} silence silence {cost}".format(
                state=state,
                sil_state=silence_start_state,
                cost=-math.log(args.transition_probability),
            ),
            file=file_handle,
        )
    ...
    print(
        "{state} {sil_state} silence silence {cost}".format(
            state=speech_last_state, sil_state=silence_start_state, cost=0.0
        ),
        file=file_handle,
    )
    ...
    for state in range(speech_start_state, speech_last_state + 1):
        print(
            "{state} {cost}".format(
                state=state, cost=-math.log(1.0 - args.edge_silence_probability)
            ),
            file=file_handle,
        )
