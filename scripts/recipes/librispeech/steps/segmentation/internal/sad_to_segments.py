...
""",
    ...
    parser.add_argument("out_segments", type=str, help="Output kaldi segments file")
...
def process_label(text_label):
    ...
    if prev_label not in [1, 2]:
        raise ValueError(
            "Expecting label to 1 (non-speech) or 2 (speech); "
...
class Segmentation(object):
    ...
    def write(self, key, file_handle):
        ...
        for segment in self.segments:
            ...
            print(
                "{seg_id} {key} {st:.2f} {end:.2f}".format(
...
def run(args):
    ...
    with common_lib.smart_open(args.in_sad) as in_sad_fh, common_lib.smart_open(
        args.out_segments, "w"
    ) as out_segments_fh:
