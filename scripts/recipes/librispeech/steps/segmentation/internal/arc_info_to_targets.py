...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="File containing a list of phones that will be " "treated as silence",
    )
    parser.add_argument(
        "--garbage-phones",
        type=str,
    ...
    parser.add_argument(
        "targets_file",
        type=str,
        help="File to write targets matrix archive in text " "format",
    )
...
def run(args):
    ...
    if len(silence_phones) == 0:
        raise RuntimeError(
            "Could not find any phones in {silence}"
    ...
    if len(garbage_phones) == 0:
        raise RuntimeError(
            "Could not find any phones in {garbage}"
    ...
    with common_lib.smart_open(args.arc_info) as arc_info_reader, common_lib.smart_open(
        args.targets_file, "w"
    ) as targets_writer:
        for line in arc_info_reader:
            try:
                ...
                logger.error(
                    "Failed to process line {line} in {f}"
    ...
    if prev_utt != "":
        if len(targets) > 0:
            num_utts += 1
            common_lib.write_matrix_ascii(args.targets_file, targets, key=prev_utt)
        else:
            num_err += 1

    logger.info(
        "Wrote {num_utts} targets; failed with {num_err}"
