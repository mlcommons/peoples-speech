...
def get_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="The fraction of silence probability " "to add to speech",
    )
    parser.add_argument(
        "--speech-in-sil-weight",
        type=float,
        default=0.0,
        help="The fraction of speech probability " "to add to silence",
    )
    parser.add_argument(
        "--garbage-in-speech-weight",
        type=float,
        default=0.0,
        help="The fraction of garbage probability " "to add to speech",
    )
    parser.add_argument(
        "--garbage-in-sil-weight",
        type=float,
        default=0.0,
        help="The fraction of garbage probability " "to add to silence",
    )
    parser.add_argument(
        "--sil-scale",
        type=float,
...
def run(args):
    ...
    if args.priors is not None:
        ...
        if len(priors) != 0 and len(priors[0]) != 3:
            raise RuntimeError("Invalid dimension for priors {0}" "".format(priors))
