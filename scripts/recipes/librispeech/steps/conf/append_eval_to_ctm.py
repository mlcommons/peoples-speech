...
for utt, ctm_part in ctm.items():
    ...
    try:
        ...
        merged = [
            ctm_tup + evl_tup for ctm_tup, evl_tup in zip(ctm_part, eval_vec[utt])
        ]
        # check,
        for j in range(len(merged)):
            ...
            assert (
                hyp_wrd == ctm_wrd
            ), "We failed with words: hyp_wrd %s, ctm_wrd %s" % (
                hyp_wrd,
                ctm_wrd,
