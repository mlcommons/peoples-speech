...
parser.add_option(
    "--conf-targets",
    help="Targets file for logistic regression (no targets generated if '') [default %default]",
    default="",
)
parser.add_option(
    "--conf-feats",
    help="Feature file for logistic regression. [default %default]",
    default="",
)
parser.add_option(
    "--lattice-depth",
    help="Per-frame lattice depths, ascii-ark (optional). [default %default]",
    default="",
)
...
word_length = {
    wrd_id: float(length) for (wrd, wrd_id, filter, length, other_feats) in word_feats
}
# Prepare other_feats dict,
other_feats = {
    wrd_id: other_feats.strip()
    for (wrd, wrd_id, filter, length, other_feats) in word_feats
}
...
with open(o.conf_feats, "w") as f:
    for (utt, chan, beg, dur, wrd_id, conf, score_tag) in ctm:
        ...
        # - log of word-length,
        log_word_length = math.log(
            word_length[wrd_id]
        )  # i.e. number of phones in a word,
        ...
        if o.lattice_depth != "":
            depth_slice = depths[utt][
                int(round(100.0 * float(beg))) : int(
                    round(100.0 * (float(beg) + float(dur)))
                )
            ]
