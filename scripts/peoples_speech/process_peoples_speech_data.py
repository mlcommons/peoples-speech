...
parser.add_argument(
    ...
    type=int,
    help="Workers to process dataset.",
)
...
def convert_to_wav(input_manifest_json: Dict):
    ...
    for label, audio_path, duration in zip(
        ...
        output_manifest_jsons.append(
            {"audio_filepath": output_wav_path, "text": label, "duration": duration}
        )
...
def main():
    ...
    with ThreadPoolExecutor(args.num_workers) as executor:
        json_objs = list(
            tqdm(
                executor.map(convert_to_wav, input_manifest_jsons),
